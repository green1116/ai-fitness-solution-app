...
import { type ProcurementWinRate, computeWinRate } from "./analytics-types";
...
let cachedAnalytics: ProcurementWinRate[] | undefined;

export function buildProcurementWinRateAnalytics(): ProcurementWinRate[] {
  ...
  for (const decision of runProcurementDecisionEngine()) {
    ...
    const counter = counters.get(decision.procurementLevel) ?? {
      winCount: 0,
      lossCount: 0,
      pendingCount: 0,
    };
    incrementCounter(counter, outcome);
    counters.set(decision.procurementLevel, counter);
  }

  cachedAnalytics = [...counters.entries()]
    .map(([procurementLevel, counter]) => ({
      procurementLevel,
