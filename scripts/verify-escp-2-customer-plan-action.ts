/**
 * ESCP-2 — Customer Plan Action verification
 */
import {
...
function main() {
  console.log("=== ESCP-2 Customer Plan Action ===\n");
  ...
  const first = buildCustomerPlanAction(state);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESCP_2_ID, "ESCP-2 id");
  assert(first.capability === CUSTOMER_PLAN_ACTION_CAPABILITY, "capability");
  assert(first.version === CUSTOMER_PLAN_ACTION_VERSION, "version");
  assert(
    first.baselineTag === ESCP1_CUSTOMER_PLAN_STATE_BASELINE,
    "baseline",
  );
  assert(first.parentPack === "ESCP-1", "parent");
  assert(
    first.parentVersion === CUSTOMER_PLAN_STATE_VERSION,
    "parent version",
  );
  assert(first.parentVersion === "escp-1-customer-plan-state-1", "BASE");
  assert(
  ...
  assert(
    first.records.every((r, i) => {
      ...
      return (
        r.actionId === `escp-2:${rec.tenantId}:${rec.customerId}` &&
        r.planId === rec.planId &&
        r.customerId === rec.customerId &&
        r.tenantId === rec.tenantId &&
        r.fromStatus === rec.status &&
  ...
  assert(
    first.records.every((r) =>
      (CUSTOMER_PLAN_ACTIONS as readonly string[]).includes(r.action),
    ),
    "kinds",
  );
  assert(first.remediateCount === state.blockedCount, "blocked→remediate");
  assert(first.prepareCount === state.planningCount, "planning→prepare");
  assert(first.holdCount === state.readyCount, "ready→hold");
  ...
  const second = buildCustomerPlanAction(state);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(
    customerPlanActionFingerprint(second) === first.fingerprint,
    "helper",
  );
  assert(getCustomerPlanAction().fingerprint === first.fingerprint, "cache");
  const mutated = getCustomerPlanAction();
  (mutated as { remediateCount: number }).remediateCount = -1;
  assert(
    getCustomerPlanAction().remediateCount === first.remediateCount,
    "clone-on-get",
  );
  clearCustomerPlanAction();
  const third = buildCustomerPlanAction();
