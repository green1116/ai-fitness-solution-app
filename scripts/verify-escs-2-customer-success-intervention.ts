/**
 * ESCS-2 — Customer Success Intervention verification
 */
import {
...
function main() {
  console.log("=== ESCS-2 Customer Success Intervention ===\n");
  ...
  const first = buildCustomerSuccessIntervention(state);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESCS_2_ID, "ESCS-2 id");
  assert(
    first.capability === CUSTOMER_SUCCESS_INTERVENTION_CAPABILITY,
    "capability",
  );
  assert(first.version === CUSTOMER_SUCCESS_INTERVENTION_VERSION, "version");
  assert(
    first.baselineTag === ESCS1_CUSTOMER_SUCCESS_STATE_BASELINE,
    "baseline",
  );
  assert(first.parentPack === "ESCS-1", "parent");
  assert(
    first.parentBaseline === "enterprise-saas-customer-lifecycle-operations-v1",
    "parent baseline",
  );
  assert(
  ...
  assert(
    first.customerSuccessStateFingerprint === state.fingerprint,
    "state fp",
  );
  ...
  assert(
    first.records.every((r) =>
      (CUSTOMER_SUCCESS_INTERVENTIONS as readonly string[]).includes(
        r.intervention,
      ),
    ),
    "kinds",
  ...
  assert(first.assistCount === state.attentionCount, "attention→assist");
  assert(first.enableCount === state.growingCount, "growing→enable");
  assert(first.guideCount === state.adoptingCount, "adopting→guide");
  assert(first.holdCount === state.healthyCount, "healthy→hold");
  ...
  const second = buildCustomerSuccessIntervention(state);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(
    customerSuccessInterventionFingerprint(second) === first.fingerprint,
    "helper",
  );
  assert(
    getCustomerSuccessIntervention().fingerprint === first.fingerprint,
    "cache",
  );
  clearCustomerSuccessIntervention();
  const third = buildCustomerSuccessIntervention();
