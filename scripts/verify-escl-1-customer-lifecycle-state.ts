/**
 * ESCL-1 — Customer Lifecycle State verification
 */
import {
...
function main() {
  console.log("=== ESCL-1 Customer Lifecycle State ===\n");
  ...
  const first = buildCustomerLifecycleState(feedback);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESCL_1_ID, "ESCL-1 id");
  assert(first.capability === CUSTOMER_LIFECYCLE_STATE_CAPABILITY, "capability");
  assert(first.version === CUSTOMER_LIFECYCLE_STATE_VERSION, "version");
  assert(first.baselineTag === ESCE_V1_BASELINE, "baseline");
  assert(
    first.baselineTag === ENTERPRISE_SAAS_COMMERCIAL_EXECUTION_V1,
    "ESCE v1",
  );
  assert(first.parentPack === "ESCE-3", "parent");
  assert(
  ...
  assert(
    first.executionFeedbackFingerprint === feedback.fingerprint,
    "feedback fp",
  );
  assert(first.recordCount === feedback.recordCount, "count");
  assert(
    first.records.every(
      (r, i) =>
        r.customerId === feedback.records[i]?.customerId &&
        r.tenantId === feedback.records[i]?.tenantId &&
        r.sourceFeedback === feedback.records[i]?.feedback &&
        r.action === feedback.records[i]?.action &&
  ...
  assert(
    first.records.every((r) =>
      (CUSTOMER_LIFECYCLE_STATES as readonly string[]).includes(r.state),
    ),
    "states",
  );
  assert(first.atRiskCount >= 1, "at-risk present");
  assert(first.scope.noEsceMutation === true, "noEsceMutation");
  assert(first.fingerprint.length === 64, "fingerprint");
  console.log("PASS Build");

  const second = buildCustomerLifecycleState(feedback);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(
    customerLifecycleStateFingerprint(second) === first.fingerprint,
    "helper",
  );
  assert(
    getCustomerLifecycleState().fingerprint === first.fingerprint,
    "cache",
  );
  clearCustomerLifecycleState();
  const third = buildCustomerLifecycleState();
