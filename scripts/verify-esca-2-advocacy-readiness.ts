/**
 * ESCA-2 — Advocacy Readiness verification
 */
import {
...
function main() {
  console.log("=== ESCA-2 Advocacy Readiness ===\n");
  ...
  const first = buildAdvocacyReadiness(state);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESCA_2_ID, "ESCA-2 id");
  assert(first.capability === ADVOCACY_READINESS_CAPABILITY, "capability");
  assert(first.version === ADVOCACY_READINESS_VERSION, "version");
  assert(first.baselineTag === ESCA1_ADVOCACY_STATE_BASELINE, "baseline");
  assert(first.parentPack === "ESCA-1", "parent");
  assert(first.parentVersion === ADVOCACY_STATE_VERSION, "parent version");
  assert(first.parentVersion === "esca-1-advocacy-state-1", "BASE");
  assert(
  ...
  assert(
    first.records.every((r) =>
      (ADVOCACY_READINESSES as readonly string[]).includes(r.readiness),
    ),
    "kinds",
  );
  assert(first.activeCount === state.advocatingCount, "advocating→active");
  assert(first.eligibleCount === state.readyCount, "ready→eligible");
  ...
  const second = buildAdvocacyReadiness(state);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(advocacyReadinessFingerprint(second) === first.fingerprint, "helper");
  assert(getAdvocacyReadiness().fingerprint === first.fingerprint, "cache");
  clearAdvocacyReadiness();
  const third = buildAdvocacyReadiness();
