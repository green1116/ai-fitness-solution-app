...
export async function POST(req: NextRequest) {
  try {
    ...
    const gate = await runSaasOrgGate(req, "/api/growth-marketing/run", body, "manage_billing");
    const role = normalizeOrgRole(gate.role);

    if (role === "MEMBER") {
      return NextResponse.json(
        { ok: false, code: "GROWTH_MARKETING_DENIED", message: "OWNER or ADMIN required" },
        { status: 403 },
      );
    }

    const result = runGrowthMarketingCycle(gate.traceId);
