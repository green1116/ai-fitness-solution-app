/**
 * ESOS-2 — Real Application Read Surface verification
 */
import {
...
async function main() {
  console.log("=== ESOS-2 Real Application Read Surface ===\n");
  ...
  const expected = buildOperationsSurface();
  assert(expected.releaseId === RELEASE_ID, "release id");
  assert(expected.workPackageId === ESOS_1_ID, "surface id");
  assert(expected.capability === OPERATIONS_SURFACE_CAPABILITY, "capability");
  assert(expected.version === OPERATIONS_SURFACE_VERSION, "version");
  assert(
    expected.baselineTag === ENTERPRISE_SAAS_PRODUCTION_OPERATIONS_V1,
    "baseline",
  );
  assert(expected.espoBaseline === ESPO_V1_BASELINE, "ESPO v1 alias");
  ...
  assert(body.workPackageId === expected.workPackageId, "payload id");
  assert(body.capability === expected.capability, "payload capability");
  assert(body.version === expected.version, "payload version");
  assert(body.baselineTag === expected.baselineTag, "payload baseline");
  assert(body.fingerprint === expected.fingerprint, "payload fingerprint");
  assert(body.queue.length === expected.queue.length, "queue");
  assert(body.decisions.length === expected.decisions.length, "decisions");
  assert(body.outcomes.length === expected.outcomes.length, "outcomes");
  assert(body.feedback.length === expected.feedback.length, "feedback");
  assert(body.summary.itemCount === expected.summary.itemCount, "summary");
  assert(body.summary.openCount === expected.summary.openCount, "open");
  assert(body.summary.actCount === expected.summary.actCount, "act");
  assert(body.summary.recordedCount === expected.summary.recordedCount, "recorded");
  assert(body.summary.escalateCount === expected.summary.escalateCount, "escalate");
  assert(body.scope.viewOnly === true, "viewOnly");
  assert(body.scope.noEspoMutation === true, "noEspoMutation");
  assert(body.scope.noExecution === true, "noExecution");
  assert(body.scope.noPersistence === true, "noPersistence");
  assert(getEspoFreeze().fingerprint === freezeFp, "GET no ESPO mutation");
  console.log("PASS GET");

  const second = await GET();
  const secondBody = (await second.json()) as ReturnType<
    typeof getOperationsSurface
  >;
  assert(secondBody.fingerprint === expected.fingerprint, "deterministic GET");
  assert(queue.fingerprint.length === 64, "queue frozen");
  assert(decisions.fingerprint.length === 64, "decision frozen");
  assert(outcomes.fingerprint.length === 64, "outcome frozen");
  assert(feedback.fingerprint.length === 64, "feedback frozen");
  assert(freeze.fingerprint === freezeFp, "freeze frozen");
  console.log("PASS Deterministic");

  console.log("\n=== ESOS-2 VERDICT ===");
  console.log("STATUS: PASS");
  console.log(`fingerprint: ${body.fingerprint}`);
  console.log(`baseline: ${body.baselineTag}`);
