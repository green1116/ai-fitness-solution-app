/**
 * V75 P8 — Agent gate summary (declarative, all P1–P8 phases)
 */
import type { GateSummary, GateSummaryEntry, ReadinessReport } from "./signoff.types";
import { V75_AGENT_SIGNOFF_VERSION } from "./signoff.types";

export const AGENT_GATE_CATALOG: Omit<GateSummaryEntry, "ok" | "state">[] = [
  {
    id: "AG-P1",
    phase: "P1",
    label: "Agent inventory",
    verifyScript: "npx tsx scripts/verify-v75-p1-agent-inventory.ts",
  },
  {
    id: "AG-P2",
    phase: "P2",
    label: "Agent policy",
    verifyScript: "npx tsx scripts/verify-v75-p2-agent-policy-catalog.ts",
  },
  {
    id: "AG-P3",
    phase: "P3",
    label: "Agent context",
    verifyScript: "npx tsx scripts/verify-v75-p3-agent-context-catalog.ts",
  },
  {
    id: "AG-P4",
    phase: "P4",
    label: "Agent constraint",
    verifyScript: "npx tsx scripts/verify-v75-p4-agent-constraint-catalog.ts",
  },
  {
    id: "AG-P5",
    phase: "P5",
    label: "Agent evaluation",
    verifyScript: "npx tsx scripts/verify-v75-p5-agent-evaluation-catalog.ts",
  },
  {
    id: "AG-P6",
    phase: "P6",
    label: "Agent simulation",
    verifyScript: "npx tsx scripts/verify-v75-p6-agent-simulation-catalog.ts",
  },
  {
    id: "AG-P7",
    phase: "P7",
    label: "Agent compliance",
    verifyScript: "npx tsx scripts/verify-v75-p7-agent-compliance-catalog.ts",
  },
  {
    id: "AG-P8",
    phase: "P8",
    label: "Agent sign-off & freeze",
    verifyScript: "npx tsx scripts/verify-v75-p8-agent-signoff.ts",
  },
];

function isAllPriorPhasesReady(readiness: ReadinessReport): boolean {
  return (
    readiness.p1 &&
    readiness.p2 &&
    readiness.p3 &&
    readiness.p4 &&
    readiness.p5 &&
    readiness.p6 &&
    readiness.p7
  );
}

export function buildGateSummary(readiness: ReadinessReport): GateSummary {
  const priorPass = isAllPriorPhasesReady(readiness);

  const phaseOk: Record<string, boolean> = {
    P1: readiness.p1,
    P2: readiness.p2,
    P3: readiness.p3,
    P4: readiness.p4,
    P5: readiness.p5,
    P6: readiness.p6,
    P7: readiness.p7,
    P8: priorPass,
  };

  const gates: GateSummaryEntry[] = AGENT_GATE_CATALOG.map((gate) => {
    const ok = phaseOk[gate.phase] ?? false;
    const state = ok ? "pass" : readiness.blocked ? "blocked" : "fail";
    return { ...gate, ok, state };
  });

  const passCount = gates.filter((g) => g.ok).length;
  const failCount = gates.filter((g) => !g.ok).length;
  const allGatesPass = passCount === gates.length;

  return {
    version: V75_AGENT_SIGNOFF_VERSION,
    gateCount: gates.length,
    passCount,
    failCount,
    allGatesPass,
    gates,
    summary: [
      `agent-gates pass=${passCount}/${gates.length}`,
      `fail=${failCount}`,
      `allPass=${allGatesPass}`,
    ].join(" "),
  };
}

export function getGateSummaryByPhase(phase: string): GateSummaryEntry | undefined {
  const readiness = {
    p1: true,
    p2: true,
    p3: true,
    p4: true,
    p5: true,
    p6: true,
    p7: true,
    ready: true,
    blocked: false,
    summary: "default-ready",
  };
  return buildGateSummary(readiness).gates.find((g) => g.phase === phase);
}
