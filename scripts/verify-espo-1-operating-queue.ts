/**
 * ESPO-1 — Commercial Operating Queue verification
 */
import {
...
function main() {
  console.log("=== ESPO-1 Commercial Operating Queue ===\n");
  ...
  buildCustomerPlanPortfolio(action);
  const freeze = buildEscpFreeze();
  const freezeFp = freeze.fingerprint;
  const first = buildOperatingQueue(action);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESPO_1_ID, "ESPO-1 id");
  assert(first.capability === OPERATING_QUEUE_CAPABILITY, "capability");
  assert(first.version === OPERATING_QUEUE_VERSION, "version");
  assert(
    first.baselineTag === ENTERPRISE_SAAS_CUSTOMER_PLANNING_OPERATIONS_V1,
    "baseline",
  );
  assert(first.escpBaseline === ESCP_V1_BASELINE, "ESCP v1 alias");
  assert(
    first.escpBaseline === ENTERPRISE_SAAS_CUSTOMER_PLANNING_OPERATIONS_V1,
    "ESCP freeze",
  );
  assert(first.parentPack === "ESCP-Freeze", "parent");
  assert(first.parentVersion === "escp-freeze-1.0.0", "parent version");
  assert(
  ...
  assert(first.escpFreezeFingerprint === freeze.fingerprint, "freeze fp");
  assert(getEscpFreeze().fingerprint === freezeFp, "no ESCP freeze mutation");
  assert(first.recordCount === action.recordCount, "count");
  assert(
    first.records.every((r) => {
      const rec = action.records.find((a) => a.customerId === r.customerId);
      const src = state.records.find((s) => s.customerId === r.customerId);
      ...
      return (
        r.id === `espo-1:${rec.tenantId}:${rec.customerId}` &&
  ...
  assert(
    first.records.every((r, i) => r.ordinal === i + 1),
    "queue ordinal",
  );
  assert(
    first.records.every((r, i) => {
      ...
      return rank[prev.priority] <= rank[r.priority];
    }),
    "priority order",
  );
  assert(first.openCount === action.remediateCount, "remediate→open");
  assert(first.queuedCount === action.prepareCount, "prepare→queued");
  assert(first.watchCount === action.watchCount, "watch→watch");
  assert(first.heldCount === action.holdCount, "hold→held");
  ...
  const second = buildOperatingQueue(action);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(operatingQueueFingerprint(second) === first.fingerprint, "helper");
  assert(getOperatingQueue().fingerprint === first.fingerprint, "cache");
  const mutated = getOperatingQueue();
  (mutated as { openCount: number }).openCount = -1;
  assert(getOperatingQueue().openCount === first.openCount, "clone-on-get");
  clearOperatingQueue();
  const third = buildOperatingQueue();
  ...
  console.log(`escpBaseline: ${first.escpBaseline}`);
}
