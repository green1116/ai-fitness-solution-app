/**
 * ESXP-2 — Expansion Opportunity verification
 */
import {
...
function main() {
  console.log("=== ESXP-2 Expansion Opportunity ===\n");
  ...
  const first = buildExpansionOpportunity(state);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESXP_2_ID, "ESXP-2 id");
  assert(first.capability === EXPANSION_OPPORTUNITY_CAPABILITY, "capability");
  assert(first.version === EXPANSION_OPPORTUNITY_VERSION, "version");
  assert(first.baselineTag === ESXP1_EXPANSION_STATE_BASELINE, "baseline");
  assert(first.parentPack === "ESXP-1", "parent");
  assert(first.parentVersion === EXPANSION_STATE_VERSION, "parent version");
  assert(first.parentVersion === "esxp-1-expansion-state-1", "BASE");
  assert(
  ...
  assert(
    first.expansionStateFingerprint === state.fingerprint,
    "state fp",
  );
  ...
  assert(
    first.records.every((r, i) => {
      ...
      return (
        ...
        r.opportunity === mapped.opportunity &&
  ...
  assert(
    first.records.every((r) =>
      (EXPANSION_OPPORTUNITIES as readonly string[]).includes(r.opportunity),
    ),
    "kinds",
  );
  assert(first.activeCount === state.expandingCount, "expanding→active");
  assert(first.qualifiedCount === state.readyCount, "ready→qualified");
  assert(first.watchCount === state.notReadyCount, "not-ready→watch");
  assert(first.noneCount === state.blockedCount, "blocked→none");
  assert(first.noneCount >= 1, "none present");
  ...
  const second = buildExpansionOpportunity(state);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(
    expansionOpportunityFingerprint(second) === first.fingerprint,
    "helper",
  );
  assert(getExpansionOpportunity().fingerprint === first.fingerprint, "cache");
  clearExpansionOpportunity();
  const third = buildExpansionOpportunity();
