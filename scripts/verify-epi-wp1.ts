/**
 * EPI-WP1 — Product Intelligence View Foundation verification
 */
import {
...
function main() {
  console.log("=== EPI-WP1 Product Intelligence View ===\n");
  ...
  const first = buildProductIntelligenceView();

  assert(first.id === EPI_WP1_ID, "id");
  assert(
    first.scope.baseline === ENTERPRISE_SAAS_OPERATIONS_SURFACE_V1,
    "baseline",
  );
  assert(first.scope.parentPack === ESOS_1_ID, "parent ESOS-1");
  ...
  assert(first.status === "OPEN" || first.status === "QUEUED" || first.status === "WATCH" || first.status === "HELD", "status");
  assert(surface.summary.openCount >= 1, "open present");
  assert(first.status === "OPEN", "open projects to OPEN");
  assert(first.signals.openCount === surface.summary.openCount, "signal open");
  assert(first.signals.queuedCount === surface.summary.queuedCount, "signal queued");
  assert(first.signals.watchCount === surface.summary.watchCount, "signal watch");
  assert(first.signals.heldCount === surface.summary.heldCount, "signal held");
  assert(first.signals.escalateCount === surface.summary.escalateCount, "signal escalate");
  assert(first.attention.openCount === surface.summary.openCount, "attention open");
  assert(first.attention.escalateCount === surface.summary.escalateCount, "attention escalate");
  assert(first.recommendations.actCount === surface.summary.actCount, "rec act");
  assert(first.recommendations.recordedCount === surface.summary.recordedCount, "rec recorded");
  assert(first.generatedAt === `${ESPO_FREEZE_DATE}T00:00:00.000Z`, "generatedAt");
  assert(first.fingerprint.length === 64, "fingerprint");
  assert(first.signals.openCount === queue.openCount, "queue open");
  assert(first.recommendations.actCount === decisions.actCount, "decision act");
  assert(first.recommendations.recordedCount === outcomes.recordedCount, "outcome recorded");
  assert(first.signals.escalateCount === feedback.escalateCount, "feedback escalate");
  ...
  const second = buildProductIntelligenceView();
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(
    productIntelligenceViewFingerprint(second) === first.fingerprint,
    "helper",
  );
  assert(getProductIntelligenceView().fingerprint === first.fingerprint, "cache");
  const mutated = getProductIntelligenceView();
  (mutated as { attention: { openCount: number } }).attention.openCount = -1;
  assert(
    getProductIntelligenceView().attention.openCount === first.attention.openCount,
    "clone-on-get",
  );
  clearProductIntelligenceView();
  const third = buildProductIntelligenceView();
  ...
  console.log(`baseline: ${first.scope.baseline}`);
