/**
 * Product M12 — Agent compatibility binding registry
 */
...
import { getAgentCompatibilityPair } from "./pair.registry";
import type {
...
const bindings = new Map<string, AgentCompatibilityBinding>();
...
function cloneBinding(
  binding: AgentCompatibilityBinding,
): AgentCompatibilityBinding {
  return { ...binding, metadata: { ...binding.metadata } };
}

export function bindAgentCompatibilityPair(
  input: BindAgentCompatibilityPairInput,
): AgentCompatibilityBinding {
  ...
  if (
    !(AGENT_COMPATIBILITY_CONSTRAINTS as readonly string[]).includes(
      input.constraint,
    )
  ) {
  ...
  const pair = getAgentCompatibilityPair(pairId);
  ...
  const id = input.id?.trim() || createId("agtcmpbind");
  if (bindings.has(id)) throw new Error(`binding already exists: ${id}`);

  const now = nowIso();
  const binding: AgentCompatibilityBinding = {
    id,
    matrixId,
    pairId,
    bindingKey,
    ...
    fallbackVersionRef,
    status: AGENT_COMPATIBILITY_BINDING_STATUSES[0],
...
export function getAgentCompatibilityBinding(
  id: string,
): AgentCompatibilityBinding | undefined {
  const binding = bindings.get(id.trim());
  return binding ? cloneBinding(binding) : undefined;
}

export function listAgentCompatibilityBindings(filter?: {
  matrixId?: string;
...
export function clearAgentCompatibilityBindings(): void {
  bindings.clear();
}
