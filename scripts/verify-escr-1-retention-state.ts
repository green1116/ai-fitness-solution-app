/**
 * ESCR-1 — Retention State verification
 */
import {
...
function main() {
  console.log("=== ESCR-1 Retention State ===\n");
  ...
  const first = buildRetentionState(review);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESCR_1_ID, "ESCR-1 id");
  assert(first.capability === RETENTION_STATE_CAPABILITY, "capability");
  assert(first.version === RETENTION_STATE_VERSION, "version");
  assert(first.baselineTag === ESCS_V1_BASELINE, "baseline");
  assert(
    first.baselineTag === ENTERPRISE_SAAS_CUSTOMER_SUCCESS_V1,
    "ESCS v1",
  );
  assert(first.parentPack === "ESCS-4", "parent");
  assert(
  ...
  assert(
    first.customerSuccessReviewFingerprint === review.fingerprint,
    "review fp",
  );
  ...
  assert(
    first.records.every((r) =>
      (RETENTION_STATES as readonly string[]).includes(r.state),
    ),
    "states",
  );
  assert(first.riskCount === review.actionRequiredCount, "AR→risk");
  ...
  const second = buildRetentionState(review);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(retentionStateFingerprint(second) === first.fingerprint, "helper");
  assert(getRetentionState().fingerprint === first.fingerprint, "cache");
  clearRetentionState();
  const third = buildRetentionState();
