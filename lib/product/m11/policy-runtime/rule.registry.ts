/**
 * Product M11 — Knowledge policy rule registry (soft graphKeyRef)
 */
...
import { getKnowledgePolicy } from "./policy.registry";
import type {
  KnowledgePolicyRule,
...
const rules = new Map<string, KnowledgePolicyRule>();
...
function cloneRule(rule: KnowledgePolicyRule): KnowledgePolicyRule {
  return { ...rule, metadata: { ...rule.metadata } };
}

export function registerKnowledgePolicyRule(
  input: RegisterKnowledgePolicyRuleInput,
): KnowledgePolicyRule {
  ...
  if (
    !(KNOWLEDGE_POLICY_CONSTRAINTS as readonly string[]).includes(
      input.constraint,
    )
  ) {
  ...
  if (
    !(KNOWLEDGE_POLICY_ENFORCEMENTS as readonly string[]).includes(
      input.enforcement,
    )
  ) {
  ...
  const policy = getKnowledgePolicy(policyId);
  ...
  const id = input.id?.trim() || createId("knwpolrule");
  if (rules.has(id)) throw new Error(`rule already exists: ${id}`);

  const now = nowIso();
  const rule: KnowledgePolicyRule = {
    id,
    policyId,
    ruleKey,
    sequence: input.sequence,
    status: KNOWLEDGE_POLICY_RULE_STATUSES[0],
...
export function updateKnowledgePolicyRuleStatus(
  input: UpdateKnowledgePolicyRuleStatusInput,
): KnowledgePolicyRule {
  const ruleId = input.ruleId.trim();
  if (!ruleId) throw new Error("rule.ruleId is required");
  if (
    !(KNOWLEDGE_POLICY_RULE_STATUSES as readonly string[]).includes(
      input.status,
    )
  ) {
  ...
  const updated: KnowledgePolicyRule = {
    ...existing,
    status: input.status,
...
export function getKnowledgePolicyRule(
  id: string,
): KnowledgePolicyRule | undefined {
  const rule = rules.get(id.trim());
  return rule ? cloneRule(rule) : undefined;
}

export function listKnowledgePolicyRules(filter?: {
  policyId?: string;
...
export function clearKnowledgePolicyRules(): void {
  rules.clear();
}
