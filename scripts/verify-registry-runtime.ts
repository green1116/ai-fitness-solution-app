...
function main() {
  ...
  const hasCommandHitlScript = VERIFY_REGISTRY.some(
    (entry) => entry.npmScript === "verify:command-hitl",
  );
  const hasCommandHitlBridgeScript = VERIFY_REGISTRY.some(
    (entry) => entry.npmScript === "verify:command-hitl-bridge",
  );
  const hasCommandApiScript = VERIFY_REGISTRY.some(
    (entry) => entry.npmScript === "verify:command-api",
  );
  const hasCommandGatedOrchestratorScript = VERIFY_REGISTRY.some(
    (entry) => entry.npmScript === "verify:command-gated-orchestrator",
  );
  const hasCommandPlatformBaselineScript = VERIFY_REGISTRY.some(
    (entry) => entry.npmScript === "verify:command-platform-baseline",
  );
