...
const ACTIONS = new Set<OperatorResumeAction>([
  "auto",
  "retry",
  "resume_approve",
  "rollback_valid",
...
export async function POST(req: Request, { params }: RouteCtx) {
  return withPilotRoute(req, async (ctx) => {
    ...
    const action = String(body?.action ?? "auto").trim() as OperatorResumeAction;
    if (!ACTIONS.has(action)) {
      return NextResponse.json({ ok: false, code: "INVALID_ACTION" }, { status: 400 });
    }

    try {
      const result = await operatorResumeIntake({
        sessionId,
      ...
      const message = err instanceof Error ? err.message : "OPS_RESUME_FAILED";
      const status =
        message === "SESSION_NOT_FOUND" || message === "ORG_MISMATCH"
          ? 404
          : message === "PARTIAL_WRITE_DETECTED" ||
              message === "INVALID_OPS_ACTION" ||
