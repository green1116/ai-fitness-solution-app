/**
 * EP-1 / WP-23 — Enterprise Remedy Registry verification
 */
...
function assertShape(row: RemedyRegistry, label: string) {
  ...
  assert(row.remedyName.length > 0, `${label}.remedyName`);
  assert(
    (REMEDY_TYPES as readonly string[]).includes(row.remedyType),
    `${label}.remedyType`,
  );
...
function compareChain(prev: RemedyRegistry, curr: RemedyRegistry, i: number) {
  const keys = [
    "organizationId",
    "roleId",
    "permissionId",
...
function main() {
  console.log("=== EP-1 / WP-23 Enterprise Remedy Registry ===\n");
  ...
  const first = buildRemedyRegistry();
  ...
  assert(
    new Set(
      first.map(
        (r) =>
          `${r.organizationId}:${r.roleId}:${r.permissionId}:${r.policyId}:${r.assignmentId}:${r.notificationId}:${r.alertId}:${r.escalationId}:${r.workflowId}:${r.approvalId}:${r.reviewId}:${r.auditId}:${r.complianceId}:${r.controlId}:${r.riskId}:${r.issueId}:${r.resolutionId}:${r.remedyId}`,
      ),
    ).size === first.length,
    "unique remedy keys",
  );
  ...
  const second = buildRemedyRegistry();
  assert(
    remedyRegistryFingerprint(first) === remedyRegistryFingerprint(second),
  ...
  clearRemedyRegistry();
  const viaGet = getRemedyRegistry();
  assert(viaGet.length === first.length, "get length");
  assert(
    remedyRegistryFingerprint(viaGet) === remedyRegistryFingerprint(first),
    "get fingerprint",
  );
  const again = getRemedyRegistry();
  assert(
    remedyRegistryFingerprint(again) === remedyRegistryFingerprint(viaGet),
    "get cache stable",
  );
  console.log("PASS Get");

  assert(EP_1_ID === "EP-1", "EP-1 id");
  assert(EP_WP23_ID === "WP-23", "WP-23 id");
  assert(REMEDY_REGISTRY_CAPABILITY === "RemedyRegistry", "capability");
  assert(EP_REMEDY_REGISTRY_BASELINE === "v80-pilot-ga-1.0.0", "baseline");
  assert(
    EP_REMEDY_REGISTRY_VERSION === "ep-1-wp-23-remedy-registry-1",
    "version",
  );
  console.log("PASS EP-1 WP-23");
  ...
  console.log("\n=== ALL EP-1 / WP-23 CHECKS PASSED ===");
  console.log(
    `${EP_1_ID}/${EP_WP23_ID} · baseline ${EP_REMEDY_REGISTRY_BASELINE} · entries ${first.length}`,
  );
}
