/**
 * Product M13 — OS compatibility pair registry (soft policyKeyRef)
 */
...
const pairs = new Map<string, OsCompatibilityPair>();
...
function clonePair(pair: OsCompatibilityPair): OsCompatibilityPair {
  return { ...pair, metadata: { ...pair.metadata } };
}

export function registerOsCompatibilityPair(
  input: RegisterOsCompatibilityPairInput,
): OsCompatibilityPair {
  ...
  if (
    !(OS_COMPATIBILITY_RELATIONS as readonly string[]).includes(input.relation)
  ) {
  ...
  const id = input.id?.trim() || createId("oscmppair");
  if (pairs.has(id)) throw new Error(`pair already exists: ${id}`);

  const now = nowIso();
  const pair: OsCompatibilityPair = {
    id,
    matrixId,
    pairKey,
    sequence: input.sequence,
    status: OS_COMPATIBILITY_PAIR_STATUSES[0],
...
export function updateOsCompatibilityPairStatus(
  input: UpdateOsCompatibilityPairStatusInput,
): OsCompatibilityPair {
  const pairId = input.pairId.trim();
  if (!pairId) throw new Error("pair.pairId is required");
  if (
    !(OS_COMPATIBILITY_PAIR_STATUSES as readonly string[]).includes(
      input.status,
    )
  ) {
  ...
  const updated: OsCompatibilityPair = {
    ...existing,
    status: input.status,
...
export function getOsCompatibilityPair(
  id: string,
): OsCompatibilityPair | undefined {
  const pair = pairs.get(id.trim());
  return pair ? clonePair(pair) : undefined;
}

export function listOsCompatibilityPairs(filter?: {
  matrixId?: string;
...
export function clearOsCompatibilityPairs(): void {
  pairs.clear();
}
