/**
 * EWER-1 — Controlled Real Action verification
 */
import {
...
function main() {
  console.log("=== EWER-1 Controlled Real Action ===\n");
  ...
  const readyReview = requests.records.find(
    (r) => r.requestState === "READY" && r.intent === SUPPORTED_CONTROLLED_ACTION_INTENT,
  );
  assert(readyReview, "ready REVIEW request");
  const blocked = requests.records.find((r) => r.requestState === "BLOCKED");
  assert(blocked, "BLOCKED request");
  const otherReady = requests.records.find(
    (r) => r.requestState === "READY" && r.intent !== SUPPORTED_CONTROLLED_ACTION_INTENT,
  );
  assert(otherReady, "unsupported READY request");

  const success = executeControlledAction(readyReview);
  assert(success.workPackageId === EWER_1_ID, "ewer id");
  assert(success.version === CONTROLLED_ACTION_VERSION, "ewer version");
  assert(success.supportedIntent === "REVIEW", "single action type");
  assert(success.api === CONTROLLED_ACTION_API, "existing API");
  assert(success.result === "SUCCESS", "READY executes");
  assert(success.executed === true, "executed");
  assert(success.apiInvoked === true, "api invoked");
  assert(success.customerId === readyReview.customerId, "customer preserved");
  assert(success.actionId === readyReview.actionId, "action preserved");
  assert(success.reviewStatus !== null, "review status");
  assert(success.reviewFingerprint !== null, "review fingerprint");
  assert(
    reviewPack.records.some(
      (r) =>
        r.customerId === readyReview.customerId &&
        r.fingerprint === success.reviewFingerprint,
    ),
    "existing review record",
  );
  console.log("PASS READY executes");

  const blockedResult = executeControlledAction(blocked);
  assert(blockedResult.result === "BLOCKED", "BLOCKED result");
  assert(blockedResult.executed === false, "BLOCKED not executed");
  assert(blockedResult.apiInvoked === false, "BLOCKED no API");
  assert(blockedResult.reviewFingerprint === null, "BLOCKED no review");
  assert(blockedResult.reason === "blocked-request-not-executed", "BLOCKED reason");
  console.log("PASS BLOCKED does not execute");

  const unsupported = executeControlledAction(otherReady);
  assert(unsupported.result === "FAILED", "unsupported failed");
  assert(unsupported.executed === false, "unsupported not executed");
  assert(unsupported.apiInvoked === false, "unsupported no API");
  assert(unsupported.reason === "unsupported-intent", "unsupported reason");

  const again = executeControlledAction(readyReview);
  assert(again.fingerprint === success.fingerprint, "idempotent SUCCESS");
  const blockedAgain = executeControlledAction(blocked);
  assert(blockedAgain.fingerprint === blockedResult.fingerprint, "idempotent BLOCKED");
  const unsupportedAgain = executeControlledAction(otherReady);
  assert(unsupportedAgain.fingerprint === unsupported.fingerprint, "idempotent FAILED");
  console.log("PASS repeated execution idempotent");

  const mutated = executeControlledAction(readyReview);
  (mutated as { result: string }).result = "FAILED";
  assert(executeControlledAction(readyReview).result === "SUCCESS", "clone-on-return");
  assert(executeControlledAction(readyReview).fingerprint === success.fingerprint, "deterministic result");
  ...
  console.log(`supportedIntent: ${success.supportedIntent}`);
  console.log(`readyResult: ${success.result}`);
  console.log(`blockedResult: ${blockedResult.result}`);
}
