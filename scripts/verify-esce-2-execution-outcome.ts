/**
 * ESCE-2 — Execution Outcome verification
 */
import {
...
function main() {
  console.log("=== ESCE-2 Execution Outcome ===\n");
  ...
  const first = buildExecutionOutcome(execution);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESCE_2_ID, "ESCE-2 id");
  assert(first.capability === EXECUTION_OUTCOME_CAPABILITY, "capability");
  assert(first.version === EXECUTION_OUTCOME_VERSION, "version");
  assert(first.baselineTag === ESCE1_COMMERCIAL_EXECUTION_BASELINE, "baseline");
  assert(first.parentPack === "ESCE-1", "parent");
  assert(
  ...
  assert(
    first.commercialExecutionFingerprint === execution.fingerprint,
    "execution fp",
  );
  assert(first.recordCount === execution.recordCount, "count");
  assert(
    first.records.every((r, i) => {
      const rec = execution.records[i];
      if (!rec) return false;
      const mapped = executionOutcomeFromStatus(rec.executionStatus);
      return (
        r.customerId === rec.customerId &&
        r.tenantId === rec.tenantId &&
        r.action === rec.action &&
        r.executionStatus === rec.executionStatus &&
  ...
  assert(
    first.records.every((r) =>
      (EXECUTION_OUTCOMES as readonly string[]).includes(r.outcome),
    ),
    "outcomes",
  );
  assert(first.appliedCount === execution.recordedCount, "recorded→applied");
  assert(first.pendingCount === execution.queuedCount, "queued→pending");
  assert(first.blockedCount === execution.heldCount, "held→blocked");
  ...
  const second = buildExecutionOutcome(execution);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(executionOutcomeFingerprint(second) === first.fingerprint, "helper");
  assert(getExecutionOutcome().fingerprint === first.fingerprint, "cache");
  clearExecutionOutcome();
  const third = buildExecutionOutcome();
