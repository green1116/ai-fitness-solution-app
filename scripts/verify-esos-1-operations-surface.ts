/**
 * ESOS-1 — Operations Surface View Model verification
 */
import {
...
function main() {
  console.log("=== ESOS-1 Operations Surface ===\n");
  ...
  assert(
    first.espoBaseline === ENTERPRISE_SAAS_PRODUCTION_OPERATIONS_V1,
    "ESPO freeze",
  );
  ...
  assert(
    first.queue.every((r, i) => r.fingerprint === queue.records[i]?.fingerprint),
    "queue records",
  );
  assert(
    first.decisions.every(
      (r, i) => r.fingerprint === decisions.records[i]?.fingerprint,
    ),
    "decision records",
  );
  assert(
    first.outcomes.every(
      (r, i) => r.fingerprint === outcomes.records[i]?.fingerprint,
    ),
    "outcome records",
  );
  assert(
    first.feedback.every(
      (r, i) => r.fingerprint === feedback.records[i]?.fingerprint,
    ),
    "feedback records",
  );
  ...
  assert(first.summary.openCount >= 1, "open present");
  ...
  const second = buildOperationsSurface();
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(
    operationsSurfaceFingerprint(second) === first.fingerprint,
    "helper",
  );
  assert(getOperationsSurface().fingerprint === first.fingerprint, "cache");
  const mutated = getOperationsSurface();
  (mutated as { summary: { openCount: number } }).summary.openCount = -1;
  assert(
    getOperationsSurface().summary.openCount === first.summary.openCount,
    "clone-on-get",
  );
  clearOperationsSurface();
  const third = buildOperationsSurface();
  ...
  console.log(`espoBaseline: ${first.espoBaseline}`);
}
