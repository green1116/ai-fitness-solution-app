/**
 * ESCE-3 — Execution Feedback verification
 */
import {
...
function main() {
  console.log("=== ESCE-3 Execution Feedback ===\n");
  ...
  const first = buildExecutionFeedback(outcome);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESCE_3_ID, "ESCE-3 id");
  assert(first.capability === EXECUTION_FEEDBACK_CAPABILITY, "capability");
  assert(first.version === EXECUTION_FEEDBACK_VERSION, "version");
  assert(first.baselineTag === ESCE2_EXECUTION_OUTCOME_BASELINE, "baseline");
  assert(first.parentPack === "ESCE-2", "parent");
  assert(
  ...
  assert(
    first.executionOutcomeFingerprint === outcome.fingerprint,
    "outcome fp",
  );
  ...
  assert(
    first.records.every((r, i) => {
      ...
      if (!rec) return false;
      const mapped = executionFeedbackFromOutcome(rec.outcome);
      return (
  ...
  assert(
    first.records.every((r) =>
      (EXECUTION_FEEDBACK_STATUSES as readonly string[]).includes(r.feedback),
    ),
    "statuses",
  );
  assert(first.closedCount === outcome.appliedCount, "applied→closed");
  assert(first.watchCount === outcome.pendingCount, "pending→watch");
  assert(first.openCount === outcome.blockedCount, "blocked→open");
  assert(first.openCount >= 1, "open present");
  assert(first.scope.noLiveExecution === true, "noLiveExecution");
  assert(first.fingerprint.length === 64, "fingerprint");
  console.log("PASS Build");

  const second = buildExecutionFeedback(outcome);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(executionFeedbackFingerprint(second) === first.fingerprint, "helper");
  assert(getExecutionFeedback().fingerprint === first.fingerprint, "cache");
  clearExecutionFeedback();
  const third = buildExecutionFeedback();
