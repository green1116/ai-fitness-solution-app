/**
 * ESRN-3 — Renewal Action Signal verification
 */
import {
...
function main() {
  console.log("=== ESRN-3 Renewal Action Signal ===\n");
  ...
  const first = buildRenewalActionSignal(readiness);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESRN_3_ID, "ESRN-3 id");
  assert(first.capability === RENEWAL_ACTION_SIGNAL_CAPABILITY, "capability");
  assert(first.version === RENEWAL_ACTION_SIGNAL_VERSION, "version");
  assert(first.baselineTag === ESRN2_RENEWAL_READINESS_BASELINE, "baseline");
  assert(first.parentPack === "ESRN-2", "parent");
  assert(first.parentVersion === RENEWAL_READINESS_VERSION, "parent version");
  assert(first.parentVersion === "esrn-2-renewal-readiness-1", "BASE");
  assert(
  ...
  assert(
    first.renewalReadinessFingerprint === readiness.fingerprint,
    "readiness fp",
  );
  assert(first.renewalStateFingerprint === state.fingerprint, "state fp");
  ...
  assert(
    first.records.every((r) =>
      (RENEWAL_ACTION_SIGNALS as readonly string[]).includes(r.action),
    ),
    "kinds",
  );
  ...
  assert(first.renewCount === readiness.activeCount, "active→renew");
  ...
  const second = buildRenewalActionSignal(readiness);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(
    renewalActionSignalFingerprint(second) === first.fingerprint,
    "helper",
  );
  assert(getRenewalActionSignal().fingerprint === first.fingerprint, "cache");
  clearRenewalActionSignal();
  const third = buildRenewalActionSignal();
