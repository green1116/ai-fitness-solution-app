/**
 * Product M12 — Agent policy rule registry (soft graphKeyRef)
 */
...
const rules = new Map<string, AgentPolicyRule>();
...
function cloneRule(rule: AgentPolicyRule): AgentPolicyRule {
  return { ...rule, metadata: { ...rule.metadata } };
}

export function registerAgentPolicyRule(
  input: RegisterAgentPolicyRuleInput,
): AgentPolicyRule {
  ...
  if (
    !(AGENT_POLICY_CONSTRAINTS as readonly string[]).includes(input.constraint)
  ) {
    throw new Error(`invalid rule constraint: ${input.constraint}`);
  }
  if (
    !(AGENT_POLICY_ENFORCEMENTS as readonly string[]).includes(
      input.enforcement,
    )
  ) {
  ...
  const policy = getAgentPolicy(policyId);
  ...
  const id = input.id?.trim() || createId("agtpolrule");
  if (rules.has(id)) throw new Error(`rule already exists: ${id}`);

  const now = nowIso();
  const rule: AgentPolicyRule = {
    id,
    policyId,
    ruleKey,
    sequence: input.sequence,
    status: AGENT_POLICY_RULE_STATUSES[0],
...
export function updateAgentPolicyRuleStatus(
  input: UpdateAgentPolicyRuleStatusInput,
): AgentPolicyRule {
  const ruleId = input.ruleId.trim();
  if (!ruleId) throw new Error("rule.ruleId is required");
  if (
    !(AGENT_POLICY_RULE_STATUSES as readonly string[]).includes(input.status)
  ) {
...
export function getAgentPolicyRule(id: string): AgentPolicyRule | undefined {
  const rule = rules.get(id.trim());
  return rule ? cloneRule(rule) : undefined;
}

export function listAgentPolicyRules(filter?: {
  policyId?: string;
...
export function clearAgentPolicyRules(): void {
  rules.clear();
}
