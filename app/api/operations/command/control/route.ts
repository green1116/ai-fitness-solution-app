import { postCommandControl } from "@/lib/operations/command/api/handlers";
import type { CommandControlAction } from "@/lib/operations/command/api/types";
import {
  commandApiError,
  commandApiResponse,
  deploymentIdFromRequest,
  parseCommandApiBody,
} from "@/lib/operations/command/api/route-utils";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

const CONTROL_ACTIONS: CommandControlAction[] = [
  "suspend",
  "cancel",
  "override",
  "escalate",
  "rollback-request",
];

export async function POST(request: Request) {
  const body = await parseCommandApiBody<{
    deploymentId?: string;
    intentId?: string;
    action?: CommandControlAction;
    operator?: string;
    reason?: string;
  }>(request);
  if (!body?.intentId || !body.action) return commandApiError("intentId and action required");
  if (!CONTROL_ACTIONS.includes(body.action)) return commandApiError("invalid-action");

  return commandApiResponse(
    postCommandControl({
      deploymentId: deploymentIdFromRequest(request, body),
      intentId: body.intentId,
      action: body.action,
      operator: body.operator,
      reason: body.reason,
    }),
  );
}
