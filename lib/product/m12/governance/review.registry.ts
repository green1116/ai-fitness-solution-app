/**
 * Product M12 — Agent governance review registry (soft matrixKeyRef)
 */
...
const reviews = new Map<string, AgentGovernanceReview>();
...
function cloneReview(review: AgentGovernanceReview): AgentGovernanceReview {
  return { ...review, metadata: { ...review.metadata } };
}

export function registerAgentGovernanceReview(
  input: RegisterAgentGovernanceReviewInput,
): AgentGovernanceReview {
  ...
  if (
    !(AGENT_GOVERNANCE_APPROVALS as readonly string[]).includes(input.approval)
  ) {
    throw new Error(`invalid review approval: ${input.approval}`);
  }
  if (
    !(AGENT_GOVERNANCE_RISK_LEVELS as readonly string[]).includes(
      input.riskLevel,
    )
  ) {
  ...
  const id = input.id?.trim() || createId("agtgovrev");
  if (reviews.has(id)) throw new Error(`review already exists: ${id}`);

  const now = nowIso();
  const review: AgentGovernanceReview = {
    id,
    standardId,
    reviewKey,
    sequence: input.sequence,
    status: AGENT_GOVERNANCE_REVIEW_STATUSES[0],
...
export function updateAgentGovernanceReviewStatus(
  input: UpdateAgentGovernanceReviewStatusInput,
): AgentGovernanceReview {
  const reviewId = input.reviewId.trim();
  if (!reviewId) throw new Error("review.reviewId is required");
  if (
    !(AGENT_GOVERNANCE_REVIEW_STATUSES as readonly string[]).includes(
      input.status,
    )
  ) {
  ...
  const updated: AgentGovernanceReview = {
    ...existing,
    status: input.status,
...
export function getAgentGovernanceReview(
  id: string,
): AgentGovernanceReview | undefined {
  const review = reviews.get(id.trim());
  return review ? cloneReview(review) : undefined;
}

export function listAgentGovernanceReviews(filter?: {
  standardId?: string;
...
export function clearAgentGovernanceReviews(): void {
  reviews.clear();
}
