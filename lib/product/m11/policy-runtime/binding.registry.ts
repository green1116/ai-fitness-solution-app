/**
 * Product M11 — Knowledge policy binding registry (soft edgeKeyRef)
 */

import { KNOWLEDGE_POLICY_BINDING_STATUSES } from "./policy.constants";
import { getKnowledgePolicy } from "./policy.registry";
import { getKnowledgePolicyRule } from "./rule.registry";
import type {
...
const bindings = new Map<string, KnowledgePolicyBinding>();
...
function cloneBinding(
  binding: KnowledgePolicyBinding,
): KnowledgePolicyBinding {
  return { ...binding, metadata: { ...binding.metadata } };
}

export function bindKnowledgePolicyRule(
  input: BindKnowledgePolicyRuleInput,
): KnowledgePolicyBinding {
  ...
  const rule = getKnowledgePolicyRule(ruleId);
  ...
  const id = input.id?.trim() || createId("knwpolbind");
  if (bindings.has(id)) throw new Error(`binding already exists: ${id}`);

  const now = nowIso();
  const binding: KnowledgePolicyBinding = {
    id,
    policyId,
    ruleId,
    bindingKey,
    edgeKeyRef,
    status: KNOWLEDGE_POLICY_BINDING_STATUSES[0],
...
export function getKnowledgePolicyBinding(
  id: string,
): KnowledgePolicyBinding | undefined {
  const binding = bindings.get(id.trim());
  return binding ? cloneBinding(binding) : undefined;
}

export function listKnowledgePolicyBindings(filter?: {
  policyId?: string;
...
export function clearKnowledgePolicyBindings(): void {
  bindings.clear();
}
