/**
 * EAC v1 Freeze — Action Consumption EAC-1 evidence verification
 */
import {
...
function main() {
  console.log("=== EAC v1 Freeze Gate ===\n");
  ...
  const first = buildEacFreeze();

  assert(first.id === EAC_FREEZE_ID, "freeze id");
  assert(first.version === EAC_FREEZE_VERSION, "freeze version");
  assert(first.freezeDate === EAC_FREEZE_DATE, "freeze date");
  assert(first.baseline === ENTERPRISE_SAAS_ACTION_DELIVERY_V1, "baseline");
  assert(first.product === ENTERPRISE_SAAS_ACTION_CONSUMPTION_V1, "product");
  assert(first.components === EAC_COMPONENTS, "components const");
  assert(first.components.length === 1, "EAC-1 only");
  assert(first.components[0]!.id === EAC_1_ID, "chain order");
  assert(
    first.components.every((c) => c.status === "frozen"),
    "all frozen",
  );
  assert(
    first.scope.chain === "DELIVERY -> CONSUMPTION -> FROZEN",
    "chain",
  );
  assert(first.scope.components === "EAC-1", "scope components");
  ...
  assert(first.eadsFreezeFingerprint === eadsFp, "EADS freeze locked");
  assert(first.componentFingerprints["EAC-1"] === consumptionFp, "EAC-1 fp");
  ...
  const second = getEacFreeze();
  assert(second.fingerprint === first.fingerprint, "deterministic get");
  const mutated = getEacFreeze();
  (mutated as { certification: string }).certification = "blocked";
  assert(getEacFreeze().certification === "certified", "clone-on-get");
  clearEacFreeze();
  const third = buildEacFreeze();
