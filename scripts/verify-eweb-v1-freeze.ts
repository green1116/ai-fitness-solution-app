/**
 * EWEB v1 Freeze — Action Execution Boundary EWEB-1 evidence verification
 */
import {
...
function main() {
  console.log("=== EWEB v1 Freeze Gate ===\n");
  ...
  const first = buildEwebFreeze();

  assert(first.id === EWEB_FREEZE_ID, "freeze id");
  assert(first.version === EWEB_FREEZE_VERSION, "freeze version");
  assert(first.freezeDate === EWEB_FREEZE_DATE, "freeze date");
  assert(first.baseline === ENTERPRISE_SAAS_WORKSPACE_ACTION_INTENT_V1, "baseline");
  assert(first.product === ENTERPRISE_SAAS_ACTION_EXECUTION_BOUNDARY_V1, "product");
  assert(first.components === EWEB_COMPONENTS, "components const");
  assert(first.components.length === 1, "EWEB-1 only");
  assert(first.components[0]!.id === EWEB_1_ID, "chain order");
  assert(
    first.components.every((c) => c.status === "frozen"),
    "all frozen",
  );
  assert(first.scope.chain === "INTENT -> EXECUTION-REQUEST -> FROZEN", "chain");
  assert(first.scope.components === "EWEB-1", "scope components");
  ...
  assert(first.ewiFreezeFingerprint === ewiFp, "EWI freeze locked");
  assert(first.componentFingerprints["EWEB-1"] === requestsFp, "EWEB-1 fp");
  ...
  const second = getEwebFreeze();
  assert(second.fingerprint === first.fingerprint, "deterministic get");
  const mutated = getEwebFreeze();
  (mutated as { certification: string }).certification = "blocked";
  assert(getEwebFreeze().certification === "certified", "clone-on-get");
  clearEwebFreeze();
  const third = buildEwebFreeze();
