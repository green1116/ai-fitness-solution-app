/**
 * EP-2 / WP-13 — Enterprise Workspace Result Registry verification
 */
...
function assertShape(row: WorkspaceResultRegistry, label: string) {
  ...
  assert(row.executionId.length > 0, `${label}.executionId`);
  assert(
    typeof row.resultId === "string" && row.resultId.length > 0,
    `${label}.resultId`,
  );
  assert(
    (WORKSPACE_RESULT_TYPES as readonly string[]).includes(row.resultType),
    `${label}.resultType`,
  );
...
function main() {
  console.log("=== EP-2 / WP-13 Enterprise Workspace Result Registry ===\n");
  ...
  const first = buildWorkspaceResultRegistry();
  ...
  assert(
    new Set(
      first.map(
        (r) =>
          `${r.workspaceId}|${r.memberId}|${r.roleId}|${r.permissionId}|${r.accessId}|${r.sessionId}|${r.eventId}|${r.activityId}|${r.taskId}|${r.queueId}|${r.assignmentId}|${r.executionId}|${r.resultId}`,
      ),
    ).size === first.length,
    "unique chain+resultId",
  );
  console.log("PASS Build");

  const second = buildWorkspaceResultRegistry();
  assert(
    workspaceResultRegistryFingerprint(first) ===
      workspaceResultRegistryFingerprint(second),
  ...
  clearWorkspaceResultRegistry();
  const viaGet = getWorkspaceResultRegistry();
  assert(viaGet.length === first.length, "get length");
  assert(
    workspaceResultRegistryFingerprint(viaGet) ===
      workspaceResultRegistryFingerprint(first),
    "get fingerprint",
  );
  const again = getWorkspaceResultRegistry();
  assert(
    workspaceResultRegistryFingerprint(again) ===
      workspaceResultRegistryFingerprint(viaGet),
    "get cache stable",
  );
  ...
  assert(EP_2_WP13_ID === "WP-13", "WP-13 id");
  assert(
    WORKSPACE_RESULT_REGISTRY_CAPABILITY === "WorkspaceResultRegistry",
    "capability",
  );
  assert(
    EP_WORKSPACE_RESULT_REGISTRY_BASELINE === "v80-pilot-ga-1.0.0",
    "baseline",
  );
  assert(
    EP_WORKSPACE_RESULT_REGISTRY_VERSION ===
      "ep-2-wp-13-workspace-result-registry-1",
    "version",
  );
  console.log("PASS EP-2 WP-13");
  ...
  console.log("\n=== ALL EP-2 / WP-13 CHECKS PASSED ===");
  console.log(
    `${EP_2_ID}/${EP_2_WP13_ID} · baseline ${EP_WORKSPACE_RESULT_REGISTRY_BASELINE} · entries ${first.length}`,
  );
}
