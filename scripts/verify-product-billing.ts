/**
 * Product Billing — Billing Foundation verification
 */
import fs from "node:fs";
import path from "node:path";

import { PLATFORM_V1_ID } from "../lib/platform/v1/platform.v1.constants";
import { buildPlatformV1Manifest } from "../lib/platform/v1/platform.manifest";
import { E12_PRODUCTIZATION_COMPLETE_ID } from "../lib/product/e12/signoff/governance.freeze.lock";
import { ENTERPRISE_LAUNCH_COMPLETE_ID } from "../lib/launch/signoff/governance.freeze.lock";
import { ENTERPRISE_EVOLUTION_COMPLETE_ID } from "../lib/evolution/signoff/governance.freeze.lock";
import { ENTERPRISE_COMMERCIALIZATION_COMPLETE_ID } from "../lib/commercialization/p8/freeze/freeze.lock";
import { ENTERPRISE_LAUNCH_READINESS_COMPLETE_ID } from "../lib/launch/readiness/l5/freeze/freeze.lock";
import { ENTERPRISE_OPERATIONS_COMPLETE_ID } from "../lib/operations/o5/freeze/freeze.lock";
import { ENTERPRISE_PRODUCT_COMPLETE_ID } from "../lib/product/complete/freeze/freeze.lock";
import { ENTERPRISE_PRODUCT_AUTH_BASELINE_ID } from "../lib/product/auth/freeze/freeze.lock";
import { PRODUCT_AUDIT_TRACEABILITY_ID } from "../lib/product/audit/security/security.constants";
import {
  BILLING_ACCOUNT_STATUSES,
  BILLING_MANAGER_STATUSES,
  BILLING_PLAN_TIERS,
  BILLING_READINESS_VERDICTS,
  INVOICE_STATUSES,
  PAYMENT_STATUSES,
  PRODUCT_BILLING_FOUNDATION_BASE,
  PRODUCT_BILLING_FOUNDATION_FREEZE_VERSION,
  PRODUCT_BILLING_FOUNDATION_ID,
  PRODUCT_BILLING_FOUNDATION_VERSION,
  PRODUCT_BILLING_FREEZE_VERSION,
} from "../lib/product/billing/foundation/foundation.constants";
import {
  assertProductBillingReleaseGatePass,
  checkProductBillingReleaseGate,
} from "../lib/product/billing/verify/product.release.gate";

const ROOT = path.resolve(__dirname, "..");

function check(cond: boolean, msg: string) {
  if (!cond) throw new Error(`ASSERT: ${msg}`);
}

function checkModules() {
  const required = [
    "lib/product/billing/foundation/foundation.constants.ts",
    "lib/product/billing/foundation/foundation.types.ts",
    "lib/product/billing/foundation/foundation.readiness.ts",
    "lib/product/billing/account/account.types.ts",
    "lib/product/billing/account/account.registry.ts",
    "lib/product/billing/plan/plan.types.ts",
    "lib/product/billing/plan/plan.registry.ts",
    "lib/product/billing/invoice/invoice.types.ts",
    "lib/product/billing/invoice/invoice.registry.ts",
    "lib/product/billing/payment/payment.types.ts",
    "lib/product/billing/payment/payment.registry.ts",
    "lib/product/billing/billing.manager.ts",
    "lib/product/billing/verify/product.release.gate.ts",
    "lib/product/billing/index.ts",
  ];
  for (const rel of required) {
    check(fs.existsSync(path.join(ROOT, rel)), `missing: ${rel}`);
  }
  console.log("✓ module structure");
}

function checkConstants() {
  check(
    PRODUCT_BILLING_FOUNDATION_ID ===
      "enterprise-product-billing-foundation-v1",
    "billing foundation id",
  );
  check(
    PRODUCT_BILLING_FOUNDATION_VERSION === "product-billing-1",
    "billing foundation version",
  );
  check(
    PRODUCT_BILLING_FOUNDATION_FREEZE_VERSION ===
      "product-billing-foundation-freeze-1",
    "billing foundation freeze",
  );
  check(
    PRODUCT_BILLING_FOUNDATION_BASE === ENTERPRISE_PRODUCT_AUTH_BASELINE_ID,
    "billing base = auth baseline",
  );
  check(
    PRODUCT_BILLING_FREEZE_VERSION === "product-billing-foundation-freeze-1",
    "billing freeze tag",
  );
  check(
    ENTERPRISE_PRODUCT_AUTH_BASELINE_ID ===
      "enterprise-product-auth-baseline-v1",
    "auth baseline preserved",
  );
  check(
    PRODUCT_AUDIT_TRACEABILITY_ID ===
      "enterprise-product-audit-traceability-v1",
    "audit preserved",
  );
  check(
    ENTERPRISE_PRODUCT_COMPLETE_ID === "enterprise-product-complete-v1",
    "product complete preserved",
  );
  check(
    ENTERPRISE_OPERATIONS_COMPLETE_ID === "enterprise-operations-complete-v1",
    "operations complete preserved",
  );
  check(
    ENTERPRISE_LAUNCH_READINESS_COMPLETE_ID ===
      "enterprise-launch-readiness-complete-v1",
    "launch readiness complete preserved",
  );
  check(
    ENTERPRISE_COMMERCIALIZATION_COMPLETE_ID ===
      "enterprise-commercialization-complete-v1",
    "commercialization complete preserved",
  );
  check(
    ENTERPRISE_EVOLUTION_COMPLETE_ID === "enterprise-evolution-complete-v1",
    "evolution complete preserved",
  );
  check(
    ENTERPRISE_LAUNCH_COMPLETE_ID === "enterprise-launch-complete-v1",
    "launch complete preserved",
  );
  check(
    E12_PRODUCTIZATION_COMPLETE_ID ===
      "enterprise-e12-productization-complete-v1",
    "e12 complete freeze preserved",
  );
  check(BILLING_ACCOUNT_STATUSES.length === 3, "account statuses");
  check(BILLING_PLAN_TIERS.length === 3, "plan tiers");
  check(INVOICE_STATUSES.length === 4, "invoice statuses");
  check(PAYMENT_STATUSES.length === 4, "payment statuses");
  check(BILLING_READINESS_VERDICTS.length === 3, "readiness verdicts");
  check(BILLING_MANAGER_STATUSES.length === 4, "manager statuses");
  check(PLATFORM_V1_ID === "enterprise-platform-v1", "platform v1 intact");

  const platform = buildPlatformV1Manifest();
  check(platform.aligned === true, "platform aligned");
  console.log("✓ constants + freeze tags");
}

function checkGate() {
  const gate = checkProductBillingReleaseGate();
  check(gate.result === "PASS", `gate pass: ${gate.summary}`);
  assertProductBillingReleaseGatePass(gate);
  console.log("✓ release gate PASS");
  console.log(`  ${gate.summary}`);
}

function main() {
  console.log("=== Product Billing Foundation ===");
  checkModules();
  checkConstants();
  checkGate();
  console.log("ALL PASS");
}

main();
