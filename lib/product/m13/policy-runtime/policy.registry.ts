/**
 * Product M13 — OS policy definition registry (in-memory)
 */

import { OS_POLICY_KINDS, OS_POLICY_STATUSES } from "./policy.constants";
import type {
  OsPolicy,
  OsPolicyKind,
  OsPolicyStatus,
...
const policies = new Map<string, OsPolicy>();
...
function clonePolicy(policy: OsPolicy): OsPolicy {
  return { ...policy, metadata: { ...policy.metadata } };
}

export function registerOsPolicy(input: RegisterOsPolicyInput): OsPolicy {
  ...
  if (!(OS_POLICY_KINDS as readonly string[]).includes(input.kind)) {
  ...
  const id = input.id?.trim() || createId("ospol");
  if (policies.has(id)) throw new Error(`policy already exists: ${id}`);

  const now = nowIso();
  const policy: OsPolicy = {
    id,
    policyKey,
    kind: input.kind,
    status: OS_POLICY_STATUSES[0],
    title,
    summary,
...
export function updateOsPolicyStatus(
  input: UpdateOsPolicyStatusInput,
): OsPolicy {
  const policyId = input.policyId.trim();
  if (!policyId) throw new Error("policy.policyId is required");
  if (!(OS_POLICY_STATUSES as readonly string[]).includes(input.status)) {
...
export function getOsPolicy(id: string): OsPolicy | undefined {
  const policy = policies.get(id.trim());
  return policy ? clonePolicy(policy) : undefined;
}

export function listOsPolicies(filter?: {
  kind?: OsPolicyKind;
...
export function clearOsPolicies(): void {
  policies.clear();
  keys.clear();
}
