/**
 * Product M10 — AI Runtime Audit trail registry (append-only record)
 */

import { AI_RUNTIME_AUDIT_TRAIL_STATUSES } from "./audit.constants";
import type {
  AiRuntimeAuditTrail,
...
import { getAiRuntimeAuditEvent } from "./event.registry";

const trails = new Map<string, AiRuntimeAuditTrail>();
...
function cloneTrail(entry: AiRuntimeAuditTrail): AiRuntimeAuditTrail {
  return { ...entry, metadata: { ...entry.metadata } };
}

export function appendAiRuntimeAuditTrail(
  input: AppendAiRuntimeAuditTrailInput,
): AiRuntimeAuditTrail {
  const eventId = input.eventId.trim();
  if (!eventId) throw new Error("trail.eventId is required");
  if (!getAiRuntimeAuditEvent(eventId)) {
  ...
  const id = input.id?.trim() || createId("airtatrl");
  if (trails.has(id)) throw new Error(`trail already exists: ${id}`);

  sequenceCounter += 1;
  const entry: AiRuntimeAuditTrail = {
    id,
    eventId,
    sequence: sequenceCounter,
    status: AI_RUNTIME_AUDIT_TRAIL_STATUSES[0],
...
export function markAiRuntimeAuditTrailStatus(
  input: MarkAiRuntimeAuditTrailStatusInput,
): AiRuntimeAuditTrail {
  ...
  const updated: AiRuntimeAuditTrail = {
    ...existing,
    status: input.status,
...
export function getAiRuntimeAuditTrail(
  id: string,
): AiRuntimeAuditTrail | undefined {
  const entry = trails.get(id.trim());
  return entry ? cloneTrail(entry) : undefined;
}

export function listAiRuntimeAuditTrails(filter?: {
...
export function clearAiRuntimeAuditTrails(): void {
  trails.clear();
  sequenceCounter = 0;
}
