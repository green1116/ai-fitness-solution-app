/**
 * ESRN-2 — Renewal Readiness verification
 */
import {
...
function main() {
  console.log("=== ESRN-2 Renewal Readiness ===\n");
  ...
  const first = buildRenewalReadiness(state);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESRN_2_ID, "ESRN-2 id");
  assert(first.capability === RENEWAL_READINESS_CAPABILITY, "capability");
  assert(first.version === RENEWAL_READINESS_VERSION, "version");
  assert(first.baselineTag === ESRN1_RENEWAL_STATE_BASELINE, "baseline");
  assert(first.parentPack === "ESRN-1", "parent");
  assert(first.parentVersion === RENEWAL_STATE_VERSION, "parent version");
  assert(first.parentVersion === "esrn-1-renewal-state-1", "BASE");
  assert(
  ...
  assert(
    first.records.every((r) =>
      (RENEWAL_READINESSES as readonly string[]).includes(r.readiness),
    ),
    "kinds",
  );
  assert(first.activeCount === state.renewingCount, "renewing→active");
  ...
  const second = buildRenewalReadiness(state);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(renewalReadinessFingerprint(second) === first.fingerprint, "helper");
  assert(getRenewalReadiness().fingerprint === first.fingerprint, "cache");
  clearRenewalReadiness();
  const third = buildRenewalReadiness();
