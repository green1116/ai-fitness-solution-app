/**
 * Product M14 — Intelligence compatibility pair registry (soft policyKeyRef)
 */
...
import { getIntelligenceCompatibilityMatrix } from "./matrix.registry";
import type {
...
const pairs = new Map<string, IntelligenceCompatibilityPair>();
...
function clonePair(
  pair: IntelligenceCompatibilityPair,
): IntelligenceCompatibilityPair {
  return { ...pair, metadata: { ...pair.metadata } };
}

export function registerIntelligenceCompatibilityPair(
  input: RegisterIntelligenceCompatibilityPairInput,
): IntelligenceCompatibilityPair {
  ...
  if (
    !(INTELLIGENCE_COMPATIBILITY_RELATIONS as readonly string[]).includes(
      input.relation,
    )
  ) {
  ...
  const matrix = getIntelligenceCompatibilityMatrix(matrixId);
  ...
  const id = input.id?.trim() || createId("intcmppair");
  if (pairs.has(id)) throw new Error(`pair already exists: ${id}`);

  const now = nowIso();
  const pair: IntelligenceCompatibilityPair = {
    id,
    matrixId,
    pairKey,
    sequence: input.sequence,
    status: INTELLIGENCE_COMPATIBILITY_PAIR_STATUSES[0],
...
export function updateIntelligenceCompatibilityPairStatus(
  input: UpdateIntelligenceCompatibilityPairStatusInput,
): IntelligenceCompatibilityPair {
  const pairId = input.pairId.trim();
  if (!pairId) throw new Error("pair.pairId is required");
  if (
    !(INTELLIGENCE_COMPATIBILITY_PAIR_STATUSES as readonly string[]).includes(
      input.status,
    )
  ) {
  ...
  const updated: IntelligenceCompatibilityPair = {
    ...existing,
    status: input.status,
...
export function getIntelligenceCompatibilityPair(
  id: string,
): IntelligenceCompatibilityPair | undefined {
  const pair = pairs.get(id.trim());
  return pair ? clonePair(pair) : undefined;
}

export function listIntelligenceCompatibilityPairs(filter?: {
  matrixId?: string;
  ...
  return result
    .slice()
    .sort((a, b) => a.sequence - b.sequence || a.pairKey.localeCompare(b.pairKey))
    .map(clonePair);
}

export function clearIntelligenceCompatibilityPairs(): void {
  pairs.clear();
}
