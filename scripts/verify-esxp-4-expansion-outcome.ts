/**
 * ESXP-4 — Expansion Outcome verification
 */
import {
...
function main() {
  console.log("=== ESXP-4 Expansion Outcome ===\n");
  ...
  const first = buildExpansionOutcome(recommendation);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESXP_4_ID, "ESXP-4 id");
  assert(first.capability === EXPANSION_OUTCOME_CAPABILITY, "capability");
  assert(first.version === EXPANSION_OUTCOME_VERSION, "version");
  assert(
    first.baselineTag === ESXP3_EXPANSION_RECOMMENDATION_BASELINE,
    "baseline",
  );
  assert(first.parentPack === "ESXP-3", "parent");
  assert(
    first.parentVersion === EXPANSION_RECOMMENDATION_VERSION,
    "parent version",
  );
  assert(first.parentVersion === "esxp-3-expansion-recommendation-1", "BASE");
  assert(
  ...
  assert(
    first.expansionRecommendationFingerprint === recommendation.fingerprint,
    "recommendation fp",
  );
  ...
  assert(
    first.records.every((r) =>
      (EXPANSION_OUTCOMES as readonly string[]).includes(r.outcome),
    ),
    "kinds",
  );
  assert(first.deferredCount === recommendation.holdCount, "hold→deferred");
  assert(first.observingCount === recommendation.monitorCount, "monitor→observing");
  assert(first.readyCount === recommendation.prepareCount, "prepare→ready");
  assert(first.inProgressCount === recommendation.pursueCount, "pursue→in-progress");
  assert(first.deferredCount >= 1, "deferred present");
  ...
  const second = buildExpansionOutcome(recommendation);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(expansionOutcomeFingerprint(second) === first.fingerprint, "helper");
  assert(getExpansionOutcome().fingerprint === first.fingerprint, "cache");
  clearExpansionOutcome();
  const third = buildExpansionOutcome();
