/**
 * Product API Governance — policy registry
 */
...
export function registerGovernancePolicy(
  input: RegisterGovernancePolicyInput,
): GovernancePolicy {
  const policyKey = input.policyKey.trim().toUpperCase();
  const title = input.title.trim();
  const portalKeyRef = input.portalKeyRef.trim().toUpperCase();
  if (!policyKey) throw new Error("policy.policyKey is required");
  if (!title) throw new Error("policy.title is required");
  if (!portalKeyRef) throw new Error("policy.portalKeyRef is required");
  if (!(GOVERNANCE_POLICY_KINDS as readonly string[]).includes(input.kind)) {
  ...
  const id = input.id?.trim() || createId("apigovpol");
  ...
  const policy: GovernancePolicy = {
    ...
    policyKey,
    kind: input.kind,
    status: GOVERNANCE_POLICY_STATUSES[0],
    title,
    portalKeyRef,
...
export function updateGovernancePolicyStatus(
  ...
  if (
    !(GOVERNANCE_POLICY_STATUSES as readonly string[]).includes(input.status)
  ) {
