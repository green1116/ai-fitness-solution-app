/**
 * WFX v1 Freeze — Workflow Experience WFX-1~WFX-2 evidence verification
 */
import {
...
async function main() {
  console.log("=== WFX v1 Freeze Gate ===\n");
  ...
  const epiFp = getEpiFreeze().fingerprint;
  const first = buildWfxFreeze();

  assert(first.id === WFX_FREEZE_ID, "freeze id");
  assert(first.version === WFX_FREEZE_VERSION, "freeze version");
  assert(first.freezeDate === WFX_FREEZE_DATE, "freeze date");
  assert(first.baseline === ENTERPRISE_SAAS_PRODUCT_EXPERIENCE_V1, "baseline");
  assert(first.product === ENTERPRISE_SAAS_WORKFLOW_EXPERIENCE_V1, "product");
  assert(first.components === WFX_COMPONENTS, "components const");
  assert(first.components.length === 2, "WFX-1~WFX-2");
  assert(
    first.components.map((c) => c.id).join(",") === `${WFX_1_ID},${WFX_2_ID}`,
    "chain order",
  );
  assert(
  ...
  assert(first.scope.chain === "LAYOUT -> PROJECTS -> FROZEN", "chain");
  ...
  assert(first.scope.noProjectFlowChanges === true, "noProjectFlowChanges");
  assert(first.pexFreezeFingerprint === pexFp, "PEX freeze locked");
  assert(first.componentFingerprints["WFX-1"] === view.fingerprint, "WFX-1 fp");
  assert(first.componentFingerprints["WFX-2"] === view.fingerprint, "WFX-2 fp");
  ...
  assert(
    readFileSync(
      join(process.cwd(), "app/(workspace)/projects/[id]/page.tsx"),
      "utf8",
    ).includes("getProjectById"),
    "detail flow unchanged",
  );
  ...
  const second = getWfxFreeze();
  assert(second.fingerprint === first.fingerprint, "deterministic get");
  const mutated = getWfxFreeze();
  (mutated as { certification: string }).certification = "blocked";
  assert(getWfxFreeze().certification === "certified", "clone-on-get");
  clearWfxFreeze();
  const third = buildWfxFreeze();
