...
export default function PilotIntakePage() {
  ...
  const onFiles = useCallback(async (files: File[]) => {
    ...
    try {
      let sid = "";
      let tid = "";
      let req: TenderRequirements | null = null;
      let rev = 0;
      for (let i = 0; i < files.length; i++) {
        ...
        sid = data.sessionId;
        tid = data.tenderIntakeId;
        req = data.requirements;
        rev = typeof data.revision === "number" ? data.revision : rev;
      }
      setSessionId(sid);
      setTenderIntakeId(tid);
      setRequirements(req);
      setExtractedRequirements(req);
      setRevision(rev || 1);
      setStep("review");
    } catch (e) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8 py-8">
      ...
      <WorkflowEntryPanelActions host="/pilot/intake" />
      ...
      {step === "upload" && (
        <section className="rounded-2xl border border-sky-800/40 bg-sky-950/10 p-8">
          <p className="mb-4 text-center text-sm text-zinc-400">
            尚未上传招标文件。可选择一个或多个 PDF / DOCX（主标书、补遗、答疑等）。
          </p>
          <label className="flex cursor-pointer flex-col items-center gap-4 rounded-xl border border-dashed border-sky-700/60 p-12 hover:border-sky-500">
            <span className="text-base font-semibold text-white">选择招标文件（可多选）</span>
            <span className="text-xs text-zinc-500">支持招标书、补遗、答疑、附件等源文件</span>
            <input
              type="file"
              multiple
      ...
      {step === "review" && requirements && sessionId && (
        <IntakeReviewEditor
          ...
          onRequirementsChange={(req, meta) => {
            setRequirements(req);
            if (typeof meta?.revision === "number") setRevision(meta.revision);
          }}
