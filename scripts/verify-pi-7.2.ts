/**
 * PI-7.2 — Product Implementation Package Routing verification runner.
 */
import { spawnSync } from "node:child_process";
import path from "node:path";

import {
  assertImplementationRoutingGate,
  runImplementationRoutingGate,
} from "../lib/implementation/verify/implementation.routing.gate";

const root = path.resolve(__dirname, "..");
const report = runImplementationRoutingGate(root);

console.log("=== PI-7.2 Implementation Package Routing ===");
console.log(
...
console.log(
  report.passed
    ? "PI-7.2 Implementation Package Routing COMPLETE — routes match foundation; existing layers reused"
    : "PI-7.2 Implementation Package Routing FAILED",
);

assertImplementationRoutingGate(report);
