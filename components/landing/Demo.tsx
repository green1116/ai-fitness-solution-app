  ...
  return (
    ...
    >
      <div className="text-center">
        ...
        <p className="mt-2 text-zinc-600">方案 · 预算 · 投标 三合一预览（无需注册）</p>
      </div>
      ...
      {result ? (
        <div className="mt-10 space-y-6">
          ...
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-5 text-center">
            <p className="text-sm font-medium text-amber-900">注册后可保存方案并解锁完整交付文件</p>
            <Link
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-3">
          <PlaceholderCard title="方案预览" desc="有氧区 · 力量区 · 面积规划" />
          <PlaceholderCard title="预算预览" desc="设备 / 安装 / 运维分项" />
          <PlaceholderCard title="投标预览" desc="合规评分 · 章节结构" />
        </div>
      )}
    </section>
  );
...
function PreviewCard({
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      <p className="text-xs font-semibold tracking-wide text-emerald-600">{title}</p>
...
function PlaceholderCard({ title, desc }: { title: string; desc: string }) {
  return (
    <div className="rounded-2xl border border-dashed border-zinc-300 bg-white/60 p-5 text-center">
      ...
      <p className="mt-4 text-xs text-emerald-600">点击「生成体验预览」查看结果</p>
    </div>
  );
}
