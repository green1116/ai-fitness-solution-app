/**
 * EADS-1 — Action Delivery Projection verification
 */
import {
...
function main() {
  console.log("=== EADS-1 Action Delivery Projection ===\n");
  ...
  const first = buildActionDeliveryItems();

  assert(first.workPackageId === EADS_1_ID, "id");
  assert(first.baselineTag === ENTERPRISE_SAAS_WORKSPACE_ACTION_V1, "baseline");
  assert(first.parentPack === EWA_3_ID, "parent");
  assert(first.parentVersion === WORKSPACE_ACTION_OUTCOME_VERSION, "parent version");
  assert(first.version === ACTION_DELIVERY_VERSION, "version");
  assert(first.recordCount === ewaOutcomes.recordCount, "record count");
  assert(first.attentionCount === ewaOutcomes.escalateCount, "attention");
  assert(
    first.visibleCount === ewaOutcomes.readyCount + ewaOutcomes.watchCount,
    "visible",
  );
  assert(first.deferredCount === ewaOutcomes.holdCount, "deferred");
  assert(
    first.records.every((r) =>
      (ACTION_DELIVERY_STATES as readonly string[]).includes(r.deliveryState),
    ),
    "deliveryState enum",
  );
  assert(
    first.records.every((r) => {
      const outcome = ewaOutcomes.records.find((o) => o.id === r.outcomeId);
      if (!outcome) return false;
      if (outcome.outcome === "ESCALATE") {
        return (
          r.deliveryState === "ATTENTION" &&
          r.reason === "delivery-attention-from-escalate"
        );
      }
      if (outcome.outcome === "READY" || outcome.outcome === "WATCH") {
        return (
          r.deliveryState === "VISIBLE" &&
          r.reason === `delivery-visible-from-${outcome.outcome.toLowerCase()}`
        );
      }
      return (
        r.deliveryState === "DEFERRED" &&
        r.reason === "delivery-deferred-from-hold"
      );
    }),
    "stable mapping",
  );
  for (let i = 1; i < first.records.length; i += 1) {
    ...
    const rank = { ATTENTION: 0, VISIBLE: 1, DEFERRED: 2 } as const;
    assert(
      rank[prev.deliveryState] < rank[cur.deliveryState] ||
        (rank[prev.deliveryState] === rank[cur.deliveryState] &&
          prev.customerId <= cur.customerId),
      "stable order",
    );
  }
  assert(first.operationsSurfaceFingerprint === surfaceFp, "ESOS fp");
  assert(first.productIntelligenceFingerprint === viewFp, "EPI fp");
  assert(first.pexFreezeFingerprint === pexFp, "PEX fp");
  assert(first.wfxFreezeFingerprint === wfxFp, "WFX fp");
  assert(first.ewaFreezeFingerprint === ewaFp, "EWA fp");
  assert(first.workspaceActionOutcomeFingerprint === outcomesFp, "EWA-3 fp");
  ...
  clearActionDeliveryItems();
  const emptyFirst = buildActionDeliveryItems({ outcomes: emptyOutcomes });
  assert(emptyFirst.recordCount === 0, "empty records");
  assert(emptyFirst.visibleCount === 0, "empty visible");
  ...
  const emptySecond = buildActionDeliveryItems({ outcomes: emptyOutcomes });
  assert(emptySecond.fingerprint === emptyFirst.fingerprint, "empty deterministic");
  console.log("PASS Empty");

  clearActionDeliveryItems();
  const second = buildActionDeliveryItems();
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(getActionDeliveryItems().fingerprint === first.fingerprint, "cache");
  const mutated = getActionDeliveryItems();
  (mutated as { attentionCount: number }).attentionCount = -1;
  assert(
    getActionDeliveryItems().attentionCount === first.attentionCount,
    "clone-on-get",
  );
  clearActionDeliveryItems();
  const third = buildActionDeliveryItems();
