...
export async function POST(req: NextRequest) {
  try {
    ...
    const skuResult = (body as { skuResult?: SKUIntelligenceResult })?.skuResult;

    if (graph?.requirements) {
      const sku = skuResult ?? buildSkuMappings(graph);
      const compliance = buildTechnicalCompliancePackage({
        graph,
        skuResult: sku,
      });
    ...
    const sku = buildSkuMappings(builtGraph);
    const compliance = buildTechnicalCompliancePackage({
      graph: builtGraph,
      parseResult: parsed,
      skuResult: sku,
    ...
    const message = err instanceof Error ? err.message : "compliance failed";
    console.error("[tender/compliance]", err);
    return json(500, {
      ok: false,
      code: "TENDER_COMPLIANCE_FAILED",
      message,
    });
  }
}
