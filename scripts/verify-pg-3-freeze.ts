/**
 * PG-3 Freeze — Commercial Growth Baseline verification
 */
import {
  GA_RELEASE_BASELINE,
...
function main() {
  console.log("=== PG-3 Freeze — Commercial Growth Baseline ===\n");
  ...
  buildRevenueLifecycleRegistry();
  buildCommercialHealth();
  const growth = buildGrowthEvidence();

  const first = buildPg3FreezeManifest();

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === PG_3_FREEZE_ID, "freeze id");
  assert(first.capability === PG_3_FREEZE_CAPABILITY, "capability");
  assert(first.version === PG_3_FREEZE_VERSION, "version");
  assert(first.codename === PG_3_FREEZE_CODENAME, "codename");
  assert(first.freezeDate === PG_3_FREEZE_DATE, "freeze date");
  assert(first.baselineTag === PG3_GROWTH_EVIDENCE_BASELINE, "baseline");
  assert(first.baselineTag === "pg3-growth-evidence-v1", "baseline literal");
  ...
  assert(first.scope.components === "PG-3.1~PG-3.3", "components scope");
  assert(first.scope.closure === "PG-3-Freeze", "closure");
  console.log("PASS Build");

  assert(first.components.length === 3, "component count");
  assert(
    first.components.map((c) => c.id).join(",") ===
      PG_3_COMPONENTS.map((c) => c.id).join(","),
    "component order",
  );
  assert(
  ...
  assert(
    first.versionReferences.parentFreeze === PG_2_FREEZE_VERSION,
    "parent freeze version",
  );
  assert(
    first.versionReferences.parentFreezeId === PG_2_FREEZE_ID,
    "parent freeze id",
  );
  assert(
  ...
  assert(
    first.verificationSummary.growthFingerprint === growth.fingerprint,
    "growth fp link",
  );
  ...
  assert(
    first.verificationSummary.growthEventCount === growth.events.length,
    "summary events",
  );
  ...
  const second = buildPg3FreezeManifest();
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(
    pg3FreezeManifestFingerprint(second) === first.fingerprint,
    "fingerprint helper",
  );
  assert(getPg3FreezeManifest().fingerprint === first.fingerprint, "get cache");

  clearPg3FreezeManifest();
  const third = buildPg3FreezeManifest();
