...
function collectPhases(
  deploymentId: string,
  freeze: ReturnType<typeof buildPlanningFreezeManifest>,
): PlanningSignoffPhase[] {
  const readiness = collectPlanningPhaseReadiness(deploymentId);

  const phaseState = (ok: boolean): PlanningSignoffPhase["state"] =>
...
export function buildPlanningSignoff(input?: {
  deploymentId?: string;
  signals?: PlanningSignoffSignals;
}): PlanningSignoffReport {
  const deploymentId = input?.deploymentId ?? "v77-planning-signoff-default";
  const freeze = buildPlanningFreezeManifest({ deploymentId, signals: input?.signals });
  const readiness = collectPlanningPhaseReadiness(deploymentId);
  ...
  const signoffState: SignoffState = {
    signedOff,
    allPhasesPass,
    finalReadinessScore: signedOff ? 100 : freeze.planningCompliance.readinessScore,
  ...
  return {
    ...
    signoffId: `planning-signoff-${deploymentId}`,
    ...
    closingSummary,
    summary: [
      `planning-signoff signedOff=${signedOff}`,
...
export function assertPlanningSignoffPass(
  report: PlanningSignoffReport,
): asserts report is PlanningSignoffReport & {
  signoffState: SignoffState & { signedOff: true };
} {
  if (!report.signoffState.signedOff) {
    throw new Error(`V77 planning sign-off not complete: ${report.summary}`);
  }
}
