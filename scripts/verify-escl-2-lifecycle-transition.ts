/**
 * ESCL-2 — Lifecycle Transition verification
 */
import {
...
function main() {
  console.log("=== ESCL-2 Lifecycle Transition ===\n");
  ...
  const first = buildLifecycleTransition(state);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESCL_2_ID, "ESCL-2 id");
  assert(first.capability === LIFECYCLE_TRANSITION_CAPABILITY, "capability");
  assert(first.version === LIFECYCLE_TRANSITION_VERSION, "version");
  assert(
    first.baselineTag === ESCL1_CUSTOMER_LIFECYCLE_STATE_BASELINE,
    "baseline",
  );
  assert(first.parentPack === "ESCL-1", "parent");
  assert(
  ...
  assert(
    first.customerLifecycleStateFingerprint === state.fingerprint,
    "state fp",
  );
  ...
  assert(
    first.records.every((r, i) => {
      ...
      return (
        ...
        r.toState === mapped.toState &&
        r.transition === mapped.transition &&
        r.action === rec.action &&
  ...
  assert(
    first.records.every((r) =>
      (LIFECYCLE_TRANSITIONS as readonly string[]).includes(r.transition),
    ),
    "kinds",
  );
  assert(first.escalateCount === state.atRiskCount, "at-risk→escalate");
  ...
  const second = buildLifecycleTransition(state);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(
    lifecycleTransitionFingerprint(second) === first.fingerprint,
    "helper",
  );
  assert(getLifecycleTransition().fingerprint === first.fingerprint, "cache");
  clearLifecycleTransition();
  const third = buildLifecycleTransition();
