/**
 * ESCI-4 — Intelligence Recommendation verification
 */
import {
...
function main() {
  console.log("=== ESCI-4 Intelligence Recommendation ===\n");
  ...
  const first = buildIntelligenceRecommendation(portfolio);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESCI_4_ID, "ESCI-4 id");
  assert(
    first.capability === INTELLIGENCE_RECOMMENDATION_CAPABILITY,
    "capability",
  );
  assert(first.version === INTELLIGENCE_RECOMMENDATION_VERSION, "version");
  assert(
    first.baselineTag === ESCI3_CUSTOMER_PORTFOLIO_INTELLIGENCE_BASELINE,
    "baseline",
  );
  assert(first.parentPack === "ESCI-3", "parent");
  assert(
    first.parentVersion === CUSTOMER_PORTFOLIO_INTELLIGENCE_VERSION,
    "parent version",
  );
  assert(
    first.parentVersion === "esci-3-customer-portfolio-intelligence-1",
    "BASE",
  );
  assert(
  ...
  assert(first.recordCount === portfolio.recordCount, "count");
  assert(
    first.records.every((r, i) => {
      const rec = portfolio.records[i];
      if (!rec) return false;
  ...
  assert(
    first.records.every((r) =>
      (INTELLIGENCE_RECOMMENDATIONS as readonly string[]).includes(
        r.recommendation,
      ),
    ),
    "kinds",
  );
  assert(first.remediateCount === portfolio.riskCount, "risk→remediate");
  assert(first.advanceCount === portfolio.growingCount, "growing→advance");
  assert(first.deferCount === portfolio.stableCount, "stable→defer");
  assert(first.observeCount === portfolio.watchCount, "watch→observe");
  ...
  assert(
    first.portfolioRecommendation === portfolioMapped.recommendation,
    "portfolio recommendation",
  );
  assert(first.remediateCount >= 1, "remediate present");
  assert(first.portfolioRecommendation === "REMEDIATE", "portfolio remediate");
  ...
  const second = buildIntelligenceRecommendation(portfolio);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(
    intelligenceRecommendationFingerprint(second) === first.fingerprint,
    "helper",
  );
  assert(
    getIntelligenceRecommendation().fingerprint === first.fingerprint,
    "cache",
  );
  clearIntelligenceRecommendation();
  const third = buildIntelligenceRecommendation();
  ...
  console.log(`portfolioRecommendation: ${first.portfolioRecommendation}`);
}
