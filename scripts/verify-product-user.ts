/**
 * Product User — User Administration verification
 */
...
} from "../lib/product/user/administration/administration.constants";
import {
  assertProductUserReleaseGatePass,
  checkProductUserReleaseGate,
} from "../lib/product/user/verify/product.release.gate";
...
function checkModules() {
  const required = [
    "lib/product/user/administration/administration.constants.ts",
    "lib/product/user/administration/administration.types.ts",
    "lib/product/user/administration/administration.readiness.ts",
    "lib/product/user/account/account.types.ts",
    "lib/product/user/account/account.registry.ts",
    "lib/product/user/membership/membership.types.ts",
    "lib/product/user/membership/membership.registry.ts",
    "lib/product/user/privilege/privilege.types.ts",
    "lib/product/user/privilege/privilege.registry.ts",
    "lib/product/user/lifecycle/lifecycle.types.ts",
    "lib/product/user/lifecycle/lifecycle.registry.ts",
    "lib/product/user/user.manager.ts",
    "lib/product/user/verify/product.release.gate.ts",
...
function checkConstants() {
  ...
  check(USER_ACCOUNT_KINDS.length === 3, "account kinds");
  check(USER_ACCOUNT_STATUSES.length === 3, "account statuses");
  check(USER_MEMBERSHIP_ROLES.length === 3, "membership roles");
  check(USER_MEMBERSHIP_STATUSES.length === 3, "membership statuses");
  check(USER_PRIVILEGE_SCOPES.length === 3, "privilege scopes");
  check(USER_LIFECYCLE_STATES.length === 4, "lifecycle states");
  check(USER_READINESS_VERDICTS.length === 3, "readiness verdicts");
  check(USER_MANAGER_STATUSES.length === 4, "manager statuses");
...
function checkGate() {
  const gate = checkProductUserReleaseGate();
  check(gate.result === "PASS", `gate pass: ${gate.summary}`);
  assertProductUserReleaseGatePass(gate);
  console.log("✓ release gate PASS");
  console.log(`  ${gate.summary}`);
}

function main() {
  console.log("=== Product User Administration ===");
  checkModules();
  checkConstants();
  checkGate();
