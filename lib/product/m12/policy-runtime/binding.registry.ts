/**
 * Product M12 — Agent policy binding registry (soft edgeKeyRef)
 */

import { AGENT_POLICY_BINDING_STATUSES } from "./policy.constants";
import { getAgentPolicy } from "./policy.registry";
import { getAgentPolicyRule } from "./rule.registry";
import type {
  AgentPolicyBinding,
...
function cloneBinding(binding: AgentPolicyBinding): AgentPolicyBinding {
  return { ...binding, metadata: { ...binding.metadata } };
}

export function bindAgentPolicyRule(
  input: BindAgentPolicyRuleInput,
): AgentPolicyBinding {
  ...
  const rule = getAgentPolicyRule(ruleId);
  ...
  const id = input.id?.trim() || createId("agtpolbind");
  if (bindings.has(id)) throw new Error(`binding already exists: ${id}`);

  const now = nowIso();
  const binding: AgentPolicyBinding = {
    id,
    policyId,
    ruleId,
    bindingKey,
    edgeKeyRef,
    status: AGENT_POLICY_BINDING_STATUSES[0],
...
export function getAgentPolicyBinding(
  id: string,
): AgentPolicyBinding | undefined {
  const binding = bindings.get(id.trim());
  return binding ? cloneBinding(binding) : undefined;
}

export function listAgentPolicyBindings(filter?: {
  policyId?: string;
...
export function clearAgentPolicyBindings(): void {
  bindings.clear();
}
