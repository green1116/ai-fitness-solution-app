/**
 * EWI v1 Freeze — Workspace Action Intent EWI-1 evidence verification
 */
import {
...
function main() {
  console.log("=== EWI v1 Freeze Gate ===\n");
  ...
  const first = buildEwiFreeze();

  assert(first.id === EWI_FREEZE_ID, "freeze id");
  assert(first.version === EWI_FREEZE_VERSION, "freeze version");
  assert(first.freezeDate === EWI_FREEZE_DATE, "freeze date");
  assert(first.baseline === ENTERPRISE_SAAS_WORKSPACE_ACTION_UI_V1, "baseline");
  assert(first.product === ENTERPRISE_SAAS_WORKSPACE_ACTION_INTENT_V1, "product");
  assert(
    first.surfaceBaseline === ENTERPRISE_SAAS_WORKSPACE_ACTION_SURFACE_V1,
    "surface baseline",
  );
  assert(first.components === EWI_COMPONENTS, "components const");
  assert(first.components.length === 1, "EWI-1 only");
  assert(first.components[0]!.id === EWI_1_ID, "chain order");
  assert(
    first.components.every((c) => c.status === "frozen"),
    "all frozen",
  );
  assert(first.scope.chain === "UI -> INTENT -> FROZEN", "chain");
  assert(first.scope.components === "EWI-1", "scope components");
  ...
  assert(first.ewasFreezeFingerprint === ewasFp, "EWAS freeze locked");
  assert(first.componentFingerprints["EWI-1"] === intentsFp, "EWI-1 fp");
  ...
  const second = getEwiFreeze();
  assert(second.fingerprint === first.fingerprint, "deterministic get");
  const mutated = getEwiFreeze();
  (mutated as { certification: string }).certification = "blocked";
  assert(getEwiFreeze().certification === "certified", "clone-on-get");
  clearEwiFreeze();
  const third = buildEwiFreeze();
