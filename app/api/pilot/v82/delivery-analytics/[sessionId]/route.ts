...
import { buildSessionMonitoringTimeline } from "@/lib/pilot/v82";
...
export async function GET(_req: Request, { params }: RouteCtx) {
  return withPortalRoute("pilot", async (ctx) => {
    ...
    try {
      const timeline = buildSessionMonitoringTimeline(sessionId, ctx.organizationId);
      return NextResponse.json({ ok: true, timeline, readOnly: true });
    } catch (err) {
