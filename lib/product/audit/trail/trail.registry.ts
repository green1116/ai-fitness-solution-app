/**
 * Product Audit — Trail registry
 */

import { getAuditEvent } from "../event/event.registry";
import { AUDIT_TRAIL_STATUSES } from "../security/security.constants";
import type {
  AppendTrailInput,
  AuditTrailEntry,
  AuditTrailStatus,
...
const trails = new Map<string, AuditTrailEntry>();
...
function cloneTrail(entry: AuditTrailEntry): AuditTrailEntry {
  return { ...entry, metadata: { ...entry.metadata } };
}

export function appendTrail(input: AppendTrailInput): AuditTrailEntry {
  const eventId = input.eventId.trim();
  if (!eventId) throw new Error("trail.eventId is required");
  if (!getAuditEvent(eventId)) {
  ...
  const id = input.id?.trim() || createId("audtrl");
...
export function markTrailStatus(
  input: MarkTrailStatusInput,
): AuditTrailEntry {
...
export function getTrail(id: string): AuditTrailEntry | undefined {
  const entry = trails.get(id.trim());
  return entry ? cloneTrail(entry) : undefined;
}

export function listTrails(filter?: {
