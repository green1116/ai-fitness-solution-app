/**
 * Product M11 — Knowledge catalog entry registry (soft entityKeyRef)
 */

import { KNOWLEDGE_CATALOG_ENTRY_STATUSES } from "./catalog.constants";
...
const entries = new Map<string, KnowledgeCatalogEntry>();
...
function cloneEntry(entry: KnowledgeCatalogEntry): KnowledgeCatalogEntry {
  return { ...entry, metadata: { ...entry.metadata } };
}

export function registerKnowledgeCatalogEntry(
  input: RegisterKnowledgeCatalogEntryInput,
): KnowledgeCatalogEntry {
  ...
  if (!entityKeyRef) throw new Error("entry.entityKeyRef is required");
  ...
  const id = input.id?.trim() || createId("knwentry");
  ...
  const entry: KnowledgeCatalogEntry = {
    ...
    status: KNOWLEDGE_CATALOG_ENTRY_STATUSES[0],
    entityKeyRef,
    summary,
...
export function updateKnowledgeCatalogEntryStatus(
  input: UpdateKnowledgeCatalogEntryStatusInput,
): KnowledgeCatalogEntry {
  const entryId = input.entryId.trim();
  if (!entryId) throw new Error("entry.entryId is required");
  if (
    !(KNOWLEDGE_CATALOG_ENTRY_STATUSES as readonly string[]).includes(
      input.status,
    )
  ) {
  ...
  const updated: KnowledgeCatalogEntry = {
    ...existing,
    status: input.status,
...
export function getKnowledgeCatalogEntry(
  id: string,
...
export function listKnowledgeCatalogEntries(filter?: {
  catalogId?: string;
...
export function clearKnowledgeCatalogEntries(): void {
  entries.clear();
}
