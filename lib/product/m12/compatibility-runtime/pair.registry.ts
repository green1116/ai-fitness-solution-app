/**
 * Product M12 — Agent compatibility pair registry (soft policyKeyRef)
 */
...
import { getAgentCompatibilityMatrix } from "./matrix.registry";
import type {
...
const pairs = new Map<string, AgentCompatibilityPair>();
...
function clonePair(pair: AgentCompatibilityPair): AgentCompatibilityPair {
  return { ...pair, metadata: { ...pair.metadata } };
}

export function registerAgentCompatibilityPair(
  input: RegisterAgentCompatibilityPairInput,
): AgentCompatibilityPair {
  ...
  if (
    !(AGENT_COMPATIBILITY_RELATIONS as readonly string[]).includes(
      input.relation,
    )
  ) {
  ...
  const matrix = getAgentCompatibilityMatrix(matrixId);
  ...
  const id = input.id?.trim() || createId("agtcmppair");
  if (pairs.has(id)) throw new Error(`pair already exists: ${id}`);

  const now = nowIso();
  const pair: AgentCompatibilityPair = {
    id,
    matrixId,
    pairKey,
    sequence: input.sequence,
    status: AGENT_COMPATIBILITY_PAIR_STATUSES[0],
...
export function updateAgentCompatibilityPairStatus(
  input: UpdateAgentCompatibilityPairStatusInput,
): AgentCompatibilityPair {
  const pairId = input.pairId.trim();
  if (!pairId) throw new Error("pair.pairId is required");
  if (
    !(AGENT_COMPATIBILITY_PAIR_STATUSES as readonly string[]).includes(
      input.status,
    )
  ) {
  ...
  const updated: AgentCompatibilityPair = {
    ...existing,
    status: input.status,
...
export function getAgentCompatibilityPair(
  id: string,
): AgentCompatibilityPair | undefined {
  const pair = pairs.get(id.trim());
  return pair ? clonePair(pair) : undefined;
}

export function listAgentCompatibilityPairs(filter?: {
  matrixId?: string;
...
export function clearAgentCompatibilityPairs(): void {
  pairs.clear();
}
