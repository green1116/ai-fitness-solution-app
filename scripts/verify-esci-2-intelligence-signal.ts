/**
 * ESCI-2 — Intelligence Signal verification
 */
import {
...
function main() {
  console.log("=== ESCI-2 Intelligence Signal ===\n");
  ...
  const first = buildIntelligenceSignal(state);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESCI_2_ID, "ESCI-2 id");
  assert(first.capability === INTELLIGENCE_SIGNAL_CAPABILITY, "capability");
  assert(first.version === INTELLIGENCE_SIGNAL_VERSION, "version");
  assert(
    first.baselineTag === ESCI1_CUSTOMER_INTELLIGENCE_STATE_BASELINE,
    "baseline",
  );
  assert(first.parentPack === "ESCI-1", "parent");
  assert(
    first.parentVersion === CUSTOMER_INTELLIGENCE_STATE_VERSION,
    "parent version",
  );
  assert(
    first.parentVersion === "esci-1-customer-intelligence-state-1",
    "BASE",
  );
  assert(
  ...
  assert(
    first.customerIntelligenceStateFingerprint === state.fingerprint,
    "state fp",
  );
  ...
  assert(
    first.records.every((r) =>
      (INTELLIGENCE_SIGNALS as readonly string[]).includes(r.signal),
    ),
    "kinds",
  );
  assert(first.escalateCount === state.riskCount, "risk→escalate");
  assert(first.enableCount === state.growingCount, "growing→enable");
  assert(first.holdCount === state.stableCount, "stable→hold");
  assert(first.monitorCount === state.watchCount, "watch→monitor");
  ...
  const second = buildIntelligenceSignal(state);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(intelligenceSignalFingerprint(second) === first.fingerprint, "helper");
  assert(getIntelligenceSignal().fingerprint === first.fingerprint, "cache");
  clearIntelligenceSignal();
  const third = buildIntelligenceSignal();
