/**
 * Product M13 — OS dependency node registry (soft catalogKeyRef)
 */

import { OS_DEPENDENCY_NODE_STATUSES } from "./dependency.constants";
...
const nodes = new Map<string, OsDependencyNode>();
...
function cloneNode(node: OsDependencyNode): OsDependencyNode {
  return { ...node, metadata: { ...node.metadata } };
}

export function registerOsDependencyNode(
  input: RegisterOsDependencyNodeInput,
): OsDependencyNode {
  ...
  const id = input.id?.trim() || createId("osdepnode");
  if (nodes.has(id)) throw new Error(`node already exists: ${id}`);

  const now = nowIso();
  const node: OsDependencyNode = {
    id,
    graphId,
    nodeKey,
    sequence: input.sequence,
    status: OS_DEPENDENCY_NODE_STATUSES[0],
    catalogKeyRef,
    summary,
...
export function updateOsDependencyNodeStatus(
  input: UpdateOsDependencyNodeStatusInput,
): OsDependencyNode {
  const nodeId = input.nodeId.trim();
  if (!nodeId) throw new Error("node.nodeId is required");
  if (
    !(OS_DEPENDENCY_NODE_STATUSES as readonly string[]).includes(input.status)
  ) {
  ...
  const updated: OsDependencyNode = {
    ...existing,
    status: input.status,
...
export function getOsDependencyNode(id: string): OsDependencyNode | undefined {
  const node = nodes.get(id.trim());
  return node ? cloneNode(node) : undefined;
}

export function listOsDependencyNodes(filter?: {
  graphId?: string;
...
export function clearOsDependencyNodes(): void {
  nodes.clear();
}
