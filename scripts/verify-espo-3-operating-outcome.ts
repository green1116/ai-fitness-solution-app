/**
 * ESPO-3 — Operating Outcome verification
 */
import {
...
function main() {
  console.log("=== ESPO-3 Operating Outcome ===\n");
  ...
  buildEscpFreeze();
  buildOperatingQueue();
  const decision = buildOperatingDecision();
  const first = buildOperatingOutcome(decision);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESPO_3_ID, "ESPO-3 id");
  assert(first.capability === OPERATING_OUTCOME_CAPABILITY, "capability");
  assert(first.version === OPERATING_OUTCOME_VERSION, "version");
  assert(first.baselineTag === ESPO2_OPERATING_DECISION_BASELINE, "baseline");
  assert(first.parentPack === "ESPO-2", "parent");
  assert(first.parentVersion === OPERATING_DECISION_VERSION, "parent version");
  assert(first.parentVersion === "espo-2-operating-decision-1", "BASE");
  assert(
  ...
  assert(
    first.operatingDecisionFingerprint === decision.fingerprint,
    "decision fp",
  );
  assert(first.recordCount === decision.recordCount, "count");
  assert(
    first.records.every((r, i) => {
      const rec = decision.records[i];
      if (!rec) return false;
      const mapped = operatingOutcomeFromDecision(rec.decision);
      return (
        r.id === rec.id.replace(/^espo-2:/, "espo-3:") &&
        r.decisionId === rec.id &&
  ...
  assert(
    first.records.every((r) =>
      (OPERATING_OUTCOMES as readonly string[]).includes(r.outcome),
    ),
    "outcomes",
  );
  assert(
  ...
  assert(first.recordedCount === decision.actCount, "act→recorded");
  assert(first.pendingCount === decision.prepareCount, "prepare→pending");
  assert(first.observedCount === decision.watchCount, "watch→observed");
  assert(first.deferredCount === decision.holdCount, "hold→deferred");
  assert(first.recordedCount >= 1, "recorded present");
  ...
  const second = buildOperatingOutcome(decision);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(
    operatingOutcomeFingerprint(second) === first.fingerprint,
    "helper",
  );
  assert(getOperatingOutcome().fingerprint === first.fingerprint, "cache");
  const mutated = getOperatingOutcome();
  (mutated as { recordedCount: number }).recordedCount = -1;
  assert(
    getOperatingOutcome().recordedCount === first.recordedCount,
    "clone-on-get",
  );
  clearOperatingOutcome();
  const third = buildOperatingOutcome();
