/**
 * Product Billing Audit — Trail registry
 */

import { getBillingAuditEvent } from "../event/event.registry";
import { BILLING_TRAIL_STATUSES } from "../traceability/traceability.constants";
import type {
...
const trails = new Map<string, BillingAuditTrail>();
...
function cloneTrail(trail: BillingAuditTrail): BillingAuditTrail {
  return { ...trail, metadata: { ...trail.metadata } };
}

export function appendBillingTrail(
  input: AppendBillingTrailInput,
): BillingAuditTrail {
  const eventId = input.eventId.trim();
  if (!eventId) throw new Error("trail.eventId is required");
  if (!getBillingAuditEvent(eventId)) {
    throw new Error(`billing audit event not found: ${eventId}`);
  }
  ...
  const id = input.id?.trim() || createId("bautrl");
  if (trails.has(id)) throw new Error(`trail already exists: ${id}`);

  sequenceCounter += 1;
  const trail: BillingAuditTrail = {
    id,
    eventId,
    sequence: sequenceCounter,
    status: BILLING_TRAIL_STATUSES[0],
...
export function markBillingTrailStatus(
  ...
  const updated: BillingAuditTrail = {
    ...existing,
    status: input.status,
...
export function getBillingTrail(id: string): BillingAuditTrail | undefined {
  const trail = trails.get(id.trim());
  return trail ? cloneTrail(trail) : undefined;
}

export function listBillingTrails(filter?: {
...
export function clearBillingTrails(): void {
  trails.clear();
  sequenceCounter = 0;
}
