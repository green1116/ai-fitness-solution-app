/**
 * EWA v1 Freeze — Workspace Action EWA-1~EWA-3 evidence verification
 */
import {
...
function main() {
  console.log("=== EWA v1 Freeze Gate ===\n");
  ...
  const contexts = buildWorkspaceActionContexts();
  const contextsFp = contexts.fingerprint;
  const ewaOutcomes = buildWorkspaceActionOutcomes();
  const outcomesFp = ewaOutcomes.fingerprint;
  const first = buildEwaFreeze();

  assert(first.id === EWA_FREEZE_ID, "freeze id");
  assert(first.version === EWA_FREEZE_VERSION, "freeze version");
  assert(first.freezeDate === EWA_FREEZE_DATE, "freeze date");
  assert(first.baseline === ENTERPRISE_SAAS_WORKFLOW_EXPERIENCE_V1, "baseline");
  assert(first.product === ENTERPRISE_SAAS_WORKSPACE_ACTION_V1, "product");
  assert(first.components === EWA_COMPONENTS, "components const");
  assert(first.components.length === 3, "EWA-1~EWA-3");
  assert(
    first.components.map((c) => c.id).join(",") ===
      `${EWA_1_ID},${EWA_2_ID},${EWA_3_ID}`,
    "chain order",
  );
  assert(
  ...
  assert(
    first.scope.chain === "ACTION -> CONTEXT -> OUTCOME -> FROZEN",
    "chain",
  );
  ...
  assert(first.wfxFreezeFingerprint === wfxFp, "WFX freeze locked");
  assert(first.componentFingerprints["EWA-1"] === actionsFp, "EWA-1 fp");
  assert(first.componentFingerprints["EWA-2"] === contextsFp, "EWA-2 fp");
  assert(first.componentFingerprints["EWA-3"] === outcomesFp, "EWA-3 fp");
  ...
  assert(getWorkspaceActionContexts().fingerprint === contextsFp, "no EWA-2 mutation");
  ...
  const second = getEwaFreeze();
  assert(second.fingerprint === first.fingerprint, "deterministic get");
  const mutated = getEwaFreeze();
  (mutated as { certification: string }).certification = "blocked";
  assert(getEwaFreeze().certification === "certified", "clone-on-get");
  clearEwaFreeze();
  const third = buildEwaFreeze();
  ...
  assert(getWorkspaceActionContexts().fingerprint === contextsFp, "still no EWA-2 mutation");
