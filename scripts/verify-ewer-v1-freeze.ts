/**
 * EWER v1 Freeze — Controlled Real Action EWER-1 evidence verification
 */
import {
...
function main() {
  console.log("=== EWER v1 Freeze Gate ===\n");
  ...
  const readyReview = requests.records.find(
    (r) =>
      ...
      r.intent === SUPPORTED_CONTROLLED_ACTION_INTENT,
  );
  assert(readyReview, "ready REVIEW");
  assert(readyReview.intent === "REVIEW", "exactly REVIEW");
  const canonical = executeControlledAction(readyReview);
  assert(canonical.result === "SUCCESS", "canonical SUCCESS");
  const first = buildEwerFreeze();

  assert(first.id === EWER_FREEZE_ID, "freeze id");
  assert(first.version === EWER_FREEZE_VERSION, "freeze version");
  assert(first.freezeDate === EWER_FREEZE_DATE, "freeze date");
  assert(first.baseline === ENTERPRISE_SAAS_ACTION_EXECUTION_BOUNDARY_V1, "baseline");
  assert(first.product === ENTERPRISE_SAAS_CONTROLLED_ACTION_V1, "product");
  assert(first.components === EWER_COMPONENTS, "components const");
  assert(first.components.length === 1, "EWER-1 only");
  assert(first.components[0]!.id === EWER_1_ID, "chain order");
  assert(
    first.components.every((c) => c.status === "frozen"),
    "all frozen",
  );
  assert(
    first.scope.chain === "EXECUTION-REQUEST -> CONTROLLED-ACTION -> FROZEN",
    "chain",
  );
  assert(first.scope.components === "EWER-1", "scope components");
  assert(first.scope.supportedAction === "REVIEW", "REVIEW only");
  assert(first.supportedIntent === "REVIEW", "supportedIntent");
  assert(first.api === CONTROLLED_ACTION_API, "existing API");
  ...
  assert(first.ewebFreezeFingerprint === ewebFp, "EWEB freeze locked");
  assert(first.ewebFreezeFingerprint === "e4a37df35cd54500b36cde9b857b700b76ec9be198ccab0c9159b6709cdd8e8d", "frozen EWEB freeze fp");
  assert(requestsFp === "e0cf47cab1ec4a1b3b0fac957b92fd9726a817d259409c6b3ad5ee7db03b9b25", "frozen EWEB-1 fp");
  assert(first.componentFingerprints["EWER-1"].length === 64, "EWER-1 fp");
  ...
  const second = getEwerFreeze();
  assert(second.fingerprint === first.fingerprint, "deterministic get");
  const mutated = getEwerFreeze();
  (mutated as { certification: string }).certification = "blocked";
  assert(getEwerFreeze().certification === "certified", "clone-on-get");
  clearEwerFreeze();
  const third = buildEwerFreeze();
