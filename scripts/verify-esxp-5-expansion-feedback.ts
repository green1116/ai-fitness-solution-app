/**
 * ESXP-5 — Expansion Feedback verification
 */
import {
...
function main() {
  console.log("=== ESXP-5 Expansion Feedback ===\n");
  ...
  const first = buildExpansionFeedback(outcome);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESXP_5_ID, "ESXP-5 id");
  assert(first.capability === EXPANSION_FEEDBACK_CAPABILITY, "capability");
  assert(first.version === EXPANSION_FEEDBACK_VERSION, "version");
  assert(first.baselineTag === ESXP4_EXPANSION_OUTCOME_BASELINE, "baseline");
  assert(first.parentPack === "ESXP-4", "parent");
  assert(first.parentVersion === EXPANSION_OUTCOME_VERSION, "parent version");
  assert(first.parentVersion === "esxp-4-expansion-outcome-1", "BASE");
  assert(
  ...
  assert(
    first.expansionOutcomeFingerprint === outcome.fingerprint,
    "outcome fp",
  );
  ...
  assert(
    first.records.every((r) =>
      (EXPANSION_FEEDBACKS as readonly string[]).includes(r.feedback),
    ),
    "kinds",
  );
  assert(first.holdCount === outcome.deferredCount, "deferred→hold");
  assert(first.watchCount === outcome.observingCount, "observing→watch");
  assert(first.enableCount === outcome.readyCount, "ready→enable");
  assert(
    first.accelerateCount === outcome.inProgressCount,
    "in-progress→accelerate",
  );
  ...
  const second = buildExpansionFeedback(outcome);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(expansionFeedbackFingerprint(second) === first.fingerprint, "helper");
  assert(getExpansionFeedback().fingerprint === first.fingerprint, "cache");
  clearExpansionFeedback();
  const third = buildExpansionFeedback();
