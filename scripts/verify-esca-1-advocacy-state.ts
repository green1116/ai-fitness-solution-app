/**
 * ESCA-1 — Advocacy State verification
 */
import {
...
function main() {
  console.log("=== ESCA-1 Advocacy State ===\n");
  ...
  buildRenewalState();
  buildRenewalReadiness();
  const signal = buildRenewalActionSignal();
  const freeze = buildEsrnFreeze();
  const freezeFp = freeze.fingerprint;
  const lifecycle = getCustomerLifecycleState();
  const first = buildAdvocacyState(signal);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESCA_1_ID, "ESCA-1 id");
  assert(first.capability === ADVOCACY_STATE_CAPABILITY, "capability");
  assert(first.version === ADVOCACY_STATE_VERSION, "version");
  assert(
    first.baselineTag === ENTERPRISE_SAAS_RENEWAL_OPERATIONS_V1,
    "baseline",
  );
  assert(first.esrnBaseline === ESRN_V1_BASELINE, "ESRN v1 alias");
  assert(
    first.esrnBaseline === ENTERPRISE_SAAS_RENEWAL_OPERATIONS_V1,
    "ESRN freeze",
  );
  assert(first.parentPack === "ESRN-Freeze", "parent");
  assert(
  ...
  assert(
    first.renewalActionSignalFingerprint === signal.fingerprint,
    "signal fp",
  );
  assert(first.esrnFreezeFingerprint === freeze.fingerprint, "freeze fp");
  assert(getEsrnFreeze().fingerprint === freezeFp, "no ESRN freeze mutation");
  ...
  assert(
    first.records.every((r) =>
      (ADVOCACY_STATES as readonly string[]).includes(r.state),
    ),
    "states",
  );
  assert(first.blockedCount >= signal.holdCount, "HOLD→blocked");
  ...
  const second = buildAdvocacyState(signal);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(advocacyStateFingerprint(second) === first.fingerprint, "helper");
  assert(getAdvocacyState().fingerprint === first.fingerprint, "cache");
  const mutated = getAdvocacyState();
  (mutated as { blockedCount: number }).blockedCount = -1;
  assert(getAdvocacyState().blockedCount === first.blockedCount, "clone-on-get");
  clearAdvocacyState();
  const third = buildAdvocacyState();
  ...
  console.log(`esrnBaseline: ${first.esrnBaseline}`);
}
