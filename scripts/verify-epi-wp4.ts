/**
 * EPI-WP4 — Product Intelligence Adoption Surface verification
 */
import {
...
async function main() {
  console.log("=== EPI-WP4 Product Intelligence Adoption Surface ===\n");
  ...
  const exposure = buildProductIntelligenceExposure();
  const first = buildProductIntelligenceAdoption();
  assert(first.id === EPI_WP4_ID, "adoption id");
  assert(first.exposure.id === EPI_WP3_ID, "reuses WP3");
  assert(first.exposure.fingerprint === exposure.fingerprint, "exposure fp");
  assert(first.exposure.endpoint === PRODUCT_INTELLIGENCE_EXPOSURE_ENDPOINT, "endpoint");
  assert(first.exposure.method === PRODUCT_INTELLIGENCE_EXPOSURE_METHOD, "method");
  assert(first.exposure.view.id === EPI_WP1_ID, "reuses WP1");
  assert(
    first.exposure.view.scope.baseline === ENTERPRISE_SAAS_OPERATIONS_SURFACE_V1,
    "baseline",
  );
  assert(first.exposure.view.scope.parentPack === ESOS_1_ID, "parent ESOS-1");
  assert(first.products === PRODUCT_INTELLIGENCE_ADOPTION_PRODUCTS, "products const");
  assert(first.products.length === 3, "existing products");
  assert(first.products[0]?.href === "/quote", "quote");
  assert(first.products[1]?.href === "/budget", "budget");
  assert(first.products[2]?.href === "/tender", "tender");
  ...
  >;
  assert(body.fingerprint === first.exposure.view.fingerprint, "adopted view");
  assert(body.id === first.exposure.view.id, "adopted id");
  ...
  assert(queue.openCount === first.exposure.view.signals.openCount, "queue frozen");
  assert(decisions.actCount === first.exposure.view.recommendations.actCount, "decision frozen");
  assert(outcomes.recordedCount === first.exposure.view.recommendations.recordedCount, "outcome frozen");
  assert(feedback.escalateCount === first.exposure.view.signals.escalateCount, "feedback frozen");
  console.log("PASS Adoption");

  const second = getProductIntelligenceAdoption();
  assert(second.fingerprint === first.fingerprint, "deterministic get");
  const mutated = getProductIntelligenceAdoption();
  (mutated as { exposure: { fingerprint: string } }).exposure.fingerprint = "mutated";
  assert(
    getProductIntelligenceAdoption().exposure.fingerprint === first.exposure.fingerprint,
    "clone-on-get",
  );
  clearProductIntelligenceAdoption();
  const third = buildProductIntelligenceAdoption();
  assert(third.fingerprint === first.fingerprint, "after clear");
  assert(getProductIntelligenceExposure().fingerprint === exposure.fingerprint, "no WP3 mutation");
  ...
  console.log(`exposureFingerprint: ${first.exposure.fingerprint}`);
  console.log(`products: ${first.products.map((p) => p.id).join(",")}`);
}
