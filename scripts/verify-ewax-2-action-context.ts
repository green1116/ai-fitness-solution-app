/**
 * EWA-2 — Action Context Mapping verification
 */
import {
...
function main() {
  console.log("=== EWA-2 Action Context Mapping ===\n");
  ...
  const actionsFp = actions.fingerprint;
  const first = buildWorkspaceActionContexts();

  assert(first.workPackageId === EWA_2_ID, "id");
  assert(first.baselineTag === EWA1_WORKSPACE_ACTION_BASELINE, "baseline");
  assert(first.parentPack === EWA_1_ID, "parent");
  assert(first.parentVersion === actions.version, "parent version");
  assert(first.version === WORKSPACE_ACTION_CONTEXT_VERSION, "version");
  assert(first.recordCount === actions.recordCount, "record count");
  assert(first.workspaceActionFingerprint === actionsFp, "EWA-1 fp");
  assert(first.complianceCount === actions.reviewCount, "compliance");
  assert(first.deliveryCount === actions.prepareCount, "delivery");
  assert(first.observabilityCount === actions.monitorCount, "observability");
  assert(first.deferredCount === actions.holdCount, "deferred");
  assert(
    first.records.every((r) =>
      (WORKSPACE_BUSINESS_CONTEXTS as readonly string[]).includes(r.context),
    ),
    "context enum",
  );
  assert(
    first.records.every((r) =>
      (WORKSPACE_CONTEXT_PRIORITIES as readonly string[]).includes(r.priority),
    ),
    "priority enum",
  );
  assert(
    first.records.every((r, i) => {
      const action = actions.records.find((a) => a.id === r.actionId);
      return (
        action !== undefined &&
        r.customerId === action.customerId &&
        r.actionType === action.actionType &&
  ...
  for (let i = 1; i < first.records.length; i += 1) {
    ...
    assert(
      rank[prev.priority] < rank[cur.priority] ||
        (rank[prev.priority] === rank[cur.priority] &&
  ...
  assert(getWorkspaceActions().fingerprint === actionsFp, "no EWA-1 mutation");
  ...
  const second = buildWorkspaceActionContexts();
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(getWorkspaceActionContexts().fingerprint === first.fingerprint, "cache");
  const mutated = getWorkspaceActionContexts();
  (mutated as { complianceCount: number }).complianceCount = -1;
  assert(
    getWorkspaceActionContexts().complianceCount === first.complianceCount,
    "clone-on-get",
  );
  clearWorkspaceActionContexts();
  const third = buildWorkspaceActionContexts();
  assert(third.fingerprint === first.fingerprint, "after clear");
  assert(getWorkspaceActions().fingerprint === actionsFp, "still no EWA-1 mutation");
