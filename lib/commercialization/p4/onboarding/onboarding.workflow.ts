/**
 * Commercialization P4 — Onboarding workflow
 */
...
export function advanceOnboardingWorkflow(
  ...
  if (previousStep !== step) completed.add(previousStep);
  completed.add(step);

  const completedSteps = ONBOARDING_STEPS.filter((s) => completed.has(s));
  const isDone = step === "GO_LIVE" && completedSteps.length === ONBOARDING_STEPS.length;
  const status = isDone
    ? "COMPLETED"
    : stepIndex(step) > stepIndex(previousStep) || completedSteps.length > 0
      ? "IN_PROGRESS"
      : plan.status === "NOT_STARTED"
        ? "IN_PROGRESS"
        : plan.status;

  updateOnboardingStep(onboardingId, step, completedSteps, status);

  const id = input.id?.trim() || createId("owf");
  if (events.has(id)) {
  ...
  const event: OnboardingWorkflowEvent = {
    ...
    step,
    previousStep,
    note: (input.note ?? `advanced ${previousStep}→${step}`).trim(),
    advancedAt: nowIso(),
  };
  events.set(id, event);
...
export function listOnboardingWorkflowEvents(filter?: {
  ...
  if (filter?.step) result = result.filter((e) => e.step === filter.step);
  return result
    .slice()
