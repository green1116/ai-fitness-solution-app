/**
 * Product API Governance — review registry (no runtime workflow)
 */

import { GOVERNANCE_REVIEW_VERDICTS } from "../management/management.constants";
import { getGovernancePolicy } from "../policy/policy.registry";
import { getGovernanceStandard } from "../standard/standard.registry";
import type {
  GovernanceReview,
...
const reviews = new Map<string, GovernanceReview>();
...
function cloneReview(review: GovernanceReview): GovernanceReview {
  return { ...review, metadata: { ...review.metadata } };
}

export function recordGovernanceReview(
  input: RecordGovernanceReviewInput,
): GovernanceReview {
  ...
  if (
    !(GOVERNANCE_REVIEW_VERDICTS as readonly string[]).includes(input.verdict)
  ) {
  ...
  const standard = getGovernanceStandard(standardId);
  ...
  const id = input.id?.trim() || createId("apigovrev");
...
export function getGovernanceReview(id: string): GovernanceReview | undefined {
  const review = reviews.get(id.trim());
  return review ? cloneReview(review) : undefined;
}

export function listGovernanceReviews(filter?: {
  policyId?: string;
...
export function clearGovernanceReviews(): void {
  reviews.clear();
}
