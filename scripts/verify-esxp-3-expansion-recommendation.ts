/**
 * ESXP-3 — Expansion Recommendation verification
 */
import {
...
function main() {
  console.log("=== ESXP-3 Expansion Recommendation ===\n");
  ...
  const first = buildExpansionRecommendation(opportunity);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESXP_3_ID, "ESXP-3 id");
  assert(
    first.capability === EXPANSION_RECOMMENDATION_CAPABILITY,
    "capability",
  );
  assert(first.version === EXPANSION_RECOMMENDATION_VERSION, "version");
  assert(
    first.baselineTag === ESXP2_EXPANSION_OPPORTUNITY_BASELINE,
    "baseline",
  );
  assert(first.parentPack === "ESXP-2", "parent");
  assert(
    first.parentVersion === EXPANSION_OPPORTUNITY_VERSION,
    "parent version",
  );
  assert(first.parentVersion === "esxp-2-expansion-opportunity-1", "BASE");
  assert(
  ...
  assert(
    first.expansionOpportunityFingerprint === opportunity.fingerprint,
    "opportunity fp",
  );
  assert(first.recordCount === opportunity.recordCount, "count");
  assert(
    first.records.every((r, i) => {
      ...
      return (
        ...
        r.opportunity === rec.opportunity &&
        r.recommendation === mapped.recommendation &&
  ...
  assert(
    first.records.every((r) =>
      (EXPANSION_RECOMMENDATIONS as readonly string[]).includes(
        r.recommendation,
      ),
    ),
    "kinds",
  );
  assert(first.holdCount === opportunity.noneCount, "none→hold");
  assert(first.monitorCount === opportunity.watchCount, "watch→monitor");
  assert(first.prepareCount === opportunity.qualifiedCount, "qualified→prepare");
  assert(first.pursueCount === opportunity.activeCount, "active→pursue");
  ...
  assert(first.scope.noEscrMutation === true, "noEscrMutation");
  assert(first.fingerprint.length === 64, "fingerprint");
  console.log("PASS Build");

  const second = buildExpansionRecommendation(opportunity);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(
    expansionRecommendationFingerprint(second) === first.fingerprint,
    "helper",
  );
  assert(
    getExpansionRecommendation().fingerprint === first.fingerprint,
    "cache",
  );
  clearExpansionRecommendation();
  const third = buildExpansionRecommendation();
