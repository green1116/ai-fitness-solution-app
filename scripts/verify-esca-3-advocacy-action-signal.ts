/**
 * ESCA-3 — Advocacy Action Signal verification
 */
import {
...
function main() {
  console.log("=== ESCA-3 Advocacy Action Signal ===\n");
  ...
  const first = buildAdvocacyActionSignal(readiness);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESCA_3_ID, "ESCA-3 id");
  assert(first.capability === ADVOCACY_ACTION_SIGNAL_CAPABILITY, "capability");
  assert(first.version === ADVOCACY_ACTION_SIGNAL_VERSION, "version");
  assert(first.baselineTag === ESCA2_ADVOCACY_READINESS_BASELINE, "baseline");
  assert(first.parentPack === "ESCA-2", "parent");
  assert(first.parentVersion === ADVOCACY_READINESS_VERSION, "parent version");
  assert(first.parentVersion === "esca-2-advocacy-readiness-1", "BASE");
  assert(
  ...
  assert(
    first.advocacyReadinessFingerprint === readiness.fingerprint,
    "readiness fp",
  );
  assert(first.advocacyStateFingerprint === state.fingerprint, "state fp");
  assert(first.recordCount === readiness.recordCount, "count");
  ...
  assert(
    first.records.every((r, i) => {
      ...
      return (
        ...
        r.fromState === src.state &&
        r.readiness === rec.readiness &&
        r.action === mapped.action &&
  ...
  assert(
    first.records.every((r) =>
      (ADVOCACY_ACTION_SIGNALS as readonly string[]).includes(r.action),
    ),
    "kinds",
  );
  assert(first.holdCount === readiness.noneCount, "none→hold");
  assert(first.watchCount === readiness.watchCount, "watch→watch");
  assert(first.prepareCount === readiness.eligibleCount, "eligible→prepare");
  assert(first.advocateCount === readiness.activeCount, "active→advocate");
  assert(first.holdCount === state.blockedCount, "blocked→hold");
  assert(first.holdCount >= 1, "hold present");
  ...
  const second = buildAdvocacyActionSignal(readiness);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(
    advocacyActionSignalFingerprint(second) === first.fingerprint,
    "helper",
  );
  assert(getAdvocacyActionSignal().fingerprint === first.fingerprint, "cache");
  clearAdvocacyActionSignal();
  const third = buildAdvocacyActionSignal();
