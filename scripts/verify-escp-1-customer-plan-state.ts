/**
 * ESCP-1 — Customer Plan State verification
 */
import {
...
function main() {
  console.log("=== ESCP-1 Customer Plan State ===\n");
  ...
  const freeze = buildEsciFreeze();
  const freezeFp = freeze.fingerprint;
  const first = buildCustomerPlanState(recommendation);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESCP_1_ID, "ESCP-1 id");
  assert(first.capability === CUSTOMER_PLAN_STATE_CAPABILITY, "capability");
  assert(first.version === CUSTOMER_PLAN_STATE_VERSION, "version");
  assert(
    first.baselineTag === ENTERPRISE_SAAS_CUSTOMER_INTELLIGENCE_OPERATIONS_V1,
    "baseline",
  );
  assert(first.esciBaseline === ESCI_V1_BASELINE, "ESCI v1 alias");
  assert(
    first.esciBaseline === ENTERPRISE_SAAS_CUSTOMER_INTELLIGENCE_OPERATIONS_V1,
    "ESCI freeze",
  );
  assert(first.parentPack === "ESCI-Freeze", "parent");
  assert(
    first.parentVersion === "esci-freeze-1.0.0",
    "parent version",
  );
  assert(
  ...
  assert(
    first.intelligenceRecommendationFingerprint === recommendation.fingerprint,
    "recommendation fp",
  );
  assert(
    first.customerPortfolioIntelligenceFingerprint === portfolio.fingerprint,
    "portfolio fp",
  );
  assert(first.esciFreezeFingerprint === freeze.fingerprint, "freeze fp");
  assert(getEsciFreeze().fingerprint === freezeFp, "no ESCI freeze mutation");
  assert(
    getCustomerPortfolioIntelligence().fingerprint === portfolio.fingerprint,
    "no portfolio mutation",
  );
  assert(first.recordCount === recommendation.recordCount, "count");
  assert(
    first.records.every((r, i) => {
      ...
      return (
        r.planId === `escp-1:${rec.tenantId}:${rec.customerId}` &&
        r.customerId === rec.customerId &&
        r.tenantId === rec.tenantId &&
        r.intelligenceState === rec.intelligenceState &&
        r.signal === rec.signal &&
        r.recommendation === rec.recommendation &&
        r.status === mapped.status &&
        r.priority === mapped.priority &&
        r.focus === mapped.focus &&
  ...
  assert(
    first.records.every((r) =>
      (CUSTOMER_PLAN_STATUSES as readonly string[]).includes(r.status),
    ),
    "statuses",
  );
  assert(
    first.records.every((r) =>
      (CUSTOMER_PLAN_PRIORITIES as readonly string[]).includes(r.priority),
    ),
    "priorities",
  );
  assert(
    first.records.every((r) =>
      (CUSTOMER_PLAN_FOCUSES as readonly string[]).includes(r.focus),
    ),
    "focuses",
  );
  assert(first.blockedCount === recommendation.remediateCount, "remediate→blocked");
  assert(first.planningCount === recommendation.advanceCount, "advance→planning");
  assert(first.readyCount === recommendation.deferCount, "defer→ready");
  assert(first.notReadyCount === recommendation.observeCount, "observe→not-ready");
  ...
  const second = buildCustomerPlanState(recommendation);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(
    customerPlanStateFingerprint(second) === first.fingerprint,
    "helper",
  );
  assert(getCustomerPlanState().fingerprint === first.fingerprint, "cache");
  const mutated = getCustomerPlanState();
  (mutated as { blockedCount: number }).blockedCount = -1;
  assert(
    getCustomerPlanState().blockedCount === first.blockedCount,
    "clone-on-get",
  );
  clearCustomerPlanState();
  const third = buildCustomerPlanState();
  ...
  console.log(`esciBaseline: ${first.esciBaseline}`);
}
