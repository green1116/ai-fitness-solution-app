/**
 * ESXP-1 — Expansion State verification
 */
import {
...
function main() {
  console.log("=== ESXP-1 Expansion State ===\n");
  ...
  const first = buildExpansionState(review);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESXP_1_ID, "ESXP-1 id");
  assert(first.capability === EXPANSION_STATE_CAPABILITY, "capability");
  assert(first.version === EXPANSION_STATE_VERSION, "version");
  assert(
    first.baselineTag === ENTERPRISE_SAAS_CUSTOMER_RETENTION_OPERATIONS_V1,
    "baseline",
  );
  assert(first.escrBaseline === ESCR_V1_BASELINE, "ESCR v1 alias");
  assert(
    first.escrBaseline === ENTERPRISE_SAAS_CUSTOMER_RETENTION_V1,
    "ESCR freeze",
  );
  assert(first.parentPack === "ESCR-4", "parent");
  assert(
  ...
  assert(
    first.retentionReviewFingerprint === review.fingerprint,
    "review fp",
  );
  ...
  assert(
    first.records.every((r, i) => {
      ...
      return (
        ...
        r.retentionState === rec.fromState &&
  ...
  assert(
    first.records.every((r) =>
      (EXPANSION_STATES as readonly string[]).includes(r.state),
    ),
    "states",
  );
  assert(first.blockedCount === review.actionRequiredCount, "AR→blocked");
  ...
  const second = buildExpansionState(review);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(expansionStateFingerprint(second) === first.fingerprint, "helper");
  assert(getExpansionState().fingerprint === first.fingerprint, "cache");
  clearExpansionState();
  const third = buildExpansionState();
  ...
  console.log(`escrBaseline: ${first.escrBaseline}`);
}
