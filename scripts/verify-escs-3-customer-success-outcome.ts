/**
 * ESCS-3 — Customer Success Outcome verification
 */
import {
...
function main() {
  console.log("=== ESCS-3 Customer Success Outcome ===\n");
  ...
  const first = buildCustomerSuccessOutcome(intervention);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESCS_3_ID, "ESCS-3 id");
  assert(first.capability === CUSTOMER_SUCCESS_OUTCOME_CAPABILITY, "capability");
  assert(first.version === CUSTOMER_SUCCESS_OUTCOME_VERSION, "version");
  assert(
    first.baselineTag === ESCS2_CUSTOMER_SUCCESS_INTERVENTION_BASELINE,
    "baseline",
  );
  assert(first.parentPack === "ESCS-2", "parent");
  assert(
  ...
  assert(
    first.customerSuccessInterventionFingerprint === intervention.fingerprint,
    "intervention fp",
  );
  ...
  assert(
    first.records.every((r) =>
      (CUSTOMER_SUCCESS_OUTCOMES as readonly string[]).includes(r.outcome),
    ),
    "kinds",
  );
  ...
  const second = buildCustomerSuccessOutcome(intervention);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(
    customerSuccessOutcomeFingerprint(second) === first.fingerprint,
    "helper",
  );
  assert(getCustomerSuccessOutcome().fingerprint === first.fingerprint, "cache");
  clearCustomerSuccessOutcome();
  const third = buildCustomerSuccessOutcome();
