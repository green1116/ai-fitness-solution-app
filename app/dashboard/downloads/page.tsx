...
export default async function DownloadsPage({ searchParams }: any) {
  ...
  // days 边界检查：防止 NaN 和超出范�?
  const rawDays = Number(searchParams?.days ?? 14);
  const days = Number.isFinite(rawDays) ? Math.min(Math.max(rawDays, 1), 90) : 14; // 1~90�?
  ...
  for (const r of chartRows) {
    ...
    // 把一些失败原因归一�?invalid（你也可以按自己�?reason 字段再扩�?
