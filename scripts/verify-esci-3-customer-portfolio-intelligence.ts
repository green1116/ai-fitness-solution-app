/**
 * ESCI-3 — Customer Portfolio Intelligence verification
 */
import {
...
function main() {
  console.log("=== ESCI-3 Customer Portfolio Intelligence ===\n");
  ...
  const first = buildCustomerPortfolioIntelligence(signal);

  assert(first.releaseId === RELEASE_ID, "release id");
  assert(first.workPackageId === ESCI_3_ID, "ESCI-3 id");
  assert(
    first.capability === CUSTOMER_PORTFOLIO_INTELLIGENCE_CAPABILITY,
    "capability",
  );
  assert(first.version === CUSTOMER_PORTFOLIO_INTELLIGENCE_VERSION, "version");
  assert(first.baselineTag === ESCI2_INTELLIGENCE_SIGNAL_BASELINE, "baseline");
  assert(first.parentPack === "ESCI-2", "parent");
  assert(first.parentVersion === INTELLIGENCE_SIGNAL_VERSION, "parent version");
  assert(first.parentVersion === "esci-2-intelligence-signal-1", "BASE");
  assert(
  ...
  assert(
    first.intelligenceSignalFingerprint === signal.fingerprint,
    "signal fp",
  );
  assert(
  ...
  assert(first.riskCount === state.riskCount, "risk count");
  assert(first.growingCount === state.growingCount, "growing count");
  assert(first.stableCount === state.stableCount, "stable count");
  assert(first.watchCount === state.watchCount, "watch count");
  assert(first.escalateCount === signal.escalateCount, "escalate count");
  assert(first.enableCount === signal.enableCount, "enable count");
  assert(first.holdCount === signal.holdCount, "hold count");
  assert(first.monitorCount === signal.monitorCount, "monitor count");
  const rolled = portfolioIntelligenceFromCounts({
    riskCount: first.riskCount,
    growingCount: first.growingCount,
    stableCount: first.stableCount,
  });
  assert(first.portfolioState === rolled.portfolioState, "portfolio state");
  assert(first.portfolioSignal === rolled.portfolioSignal, "portfolio signal");
  assert(first.riskCount >= 1, "risk present");
  assert(first.portfolioState === "RISK", "portfolio risk");
  assert(first.portfolioSignal === "ESCALATE", "portfolio escalate");
  ...
  const second = buildCustomerPortfolioIntelligence(signal);
  assert(second.fingerprint === first.fingerprint, "deterministic rebuild");
  assert(
    customerPortfolioIntelligenceFingerprint(second) === first.fingerprint,
    "helper",
  );
  assert(
    getCustomerPortfolioIntelligence().fingerprint === first.fingerprint,
    "cache",
  );
  clearCustomerPortfolioIntelligence();
  const third = buildCustomerPortfolioIntelligence();
  ...
  console.log(`portfolioState: ${first.portfolioState}`);
  console.log(`portfolioSignal: ${first.portfolioSignal}`);
}
