/**
 * EPI-WP5 — Product Intelligence Feedback Surface verification
 */
import {
...
async function main() {
  console.log("=== EPI-WP5 Product Intelligence Feedback Surface ===\n");
  ...
  const adoption = buildProductIntelligenceAdoption();
  const adoptionFp = adoption.fingerprint;
  const first = buildProductIntelligenceFeedback();
  assert(first.id === EPI_WP5_ID, "feedback id");
  assert(first.adoption.id === EPI_WP4_ID, "reuses WP4");
  assert(first.adoption.fingerprint === adoptionFp, "adoption fp");
  assert(first.adoption.exposure.id === EPI_WP3_ID, "reuses WP3");
  assert(first.adoption.exposure.view.id === EPI_WP1_ID, "reuses WP1");
  assert(
    first.adoption.exposure.view.scope.baseline ===
      ENTERPRISE_SAAS_OPERATIONS_SURFACE_V1,
    "baseline",
  );
  assert(first.adoption.exposure.view.scope.parentPack === ESOS_1_ID, "parent ESOS-1");
  assert(first.adoption.products === PRODUCT_INTELLIGENCE_ADOPTION_PRODUCTS, "products");
  assert(first.status === first.adoption.exposure.view.status, "status projection");
  assert(first.signals.openCount === first.adoption.exposure.view.signals.openCount, "signal open");
  assert(first.signals.escalateCount === first.adoption.exposure.view.signals.escalateCount, "signal escalate");
  assert(first.attention.openCount === first.adoption.exposure.view.attention.openCount, "attention open");
  assert(first.attention.escalateCount === first.adoption.exposure.view.attention.escalateCount, "attention escalate");
  ...
  assert(first.signals.openCount === queue.openCount, "queue frozen");
  assert(first.signals.escalateCount === feedback.escalateCount, "feedback frozen");
  assert(decisions.actCount === first.adoption.exposure.view.recommendations.actCount, "decision frozen");
  assert(outcomes.recordedCount === first.adoption.exposure.view.recommendations.recordedCount, "outcome frozen");
  ...
  assert(getProductIntelligenceAdoption().fingerprint === adoptionFp, "no WP4 mutation");
  ...
  >;
  assert(body.fingerprint === first.adoption.exposure.view.fingerprint, "view match");
  console.log("PASS Feedback");

  const second = getProductIntelligenceFeedback();
  assert(second.fingerprint === first.fingerprint, "deterministic get");
  const mutated = getProductIntelligenceFeedback();
  (mutated as { signals: { openCount: number } }).signals.openCount = -1;
  assert(
    getProductIntelligenceFeedback().signals.openCount === first.signals.openCount,
    "clone-on-get",
  );
  clearProductIntelligenceFeedback();
  const third = buildProductIntelligenceFeedback();
  assert(third.fingerprint === first.fingerprint, "after clear");
  assert(getProductIntelligenceAdoption().fingerprint === adoptionFp, "still no WP4 mutation");
  ...
  console.log(`adoptionFingerprint: ${first.adoption.fingerprint}`);
